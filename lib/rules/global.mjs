...
import { getSectionIds, getAsideSectionIds } from '../spec.mjs';
...
export const GLOBAL_RULES = [
  ...
  defineRule({
    ...
    check: (html) => {
      ...
      // A sub-entry carries data-sub and points at a heading inside a section,
      // not at a section. It is an index into one long section rather than a
      // second outline, so it is resolved against the document's ids and never
      // counted as a section that ought to be listed. The lint already accepts
      // these; before this, the two gates disagreed about the same TOC.
      const anchors = [...nav[1].matchAll(/<a\b([^>]*)href="#([^"]+)"/g)]
        .map((m) => ({ sub: /\bdata-sub\b/.test(m[1]), id: m[2] }));
      const linked = anchors.filter((a) => !a.sub).map((a) => a.id);
      const subLinked = anchors.filter((a) => a.sub).map((a) => a.id);
      const allIds = new Set([...html.matchAll(/\sid="([^"]+)"/g)].map((m) => m[1]));
      const straySub = subLinked.filter((id) => !allIds.has(id));
      // An aside is a section and is deliberately not in the outline: it exists
      // until the reader imports or deletes it, and listing it would rewrite
      // the table of contents on every action run. Exempt here and nowhere else
      // — an empty aside, or one whose id collides, is a defect like any
      // section's.
      const asides = new Set(getAsideSectionIds(html));
      const present = getSectionIds(html).filter((id) => !asides.has(id));
      ...
      if (straySub.length) problems.push(`sub-entries link to nothing: ${straySub.join(', ')}`);
      if (unlisted.length) problems.push(`sections not linked: ${unlisted.join(', ')}`);
      const total = linked.length + subLinked.length;
      return {
        ok: problems.length === 0,
        detail: problems.length ? problems.join('; ') : `${total} entries, all resolving`,
      };
    },
