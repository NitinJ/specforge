...
export function parseTemplateRules(html) {
  ...
  while ((m = liRe.exec(block.html))) {
    ...
    const body = m[2];
    const fix = textOf((body.match(/<span\b[^>]*class="[^"]*\bfix\b[^"]*"[^>]*>([\s\S]*?)<\/span>/i) || [, ''])[1]);
    const ask = textOf(body.replace(/<span\b[^>]*class="[^"]*\bfix\b[^"]*"[^>]*>[\s\S]*?<\/span>/gi, ' '));
    const rule = { id };
    if (severity) {
...
export function renderTemplateBlocks(shell, { rules = [], prompts = {} } = {}) {
  ...
  if (rules.length) {
    const items = rules.map((r) => {
      ...
      const fix = r.fix ? ` <span class="fix">${escapeHtml(r.fix)}</span>` : '';
