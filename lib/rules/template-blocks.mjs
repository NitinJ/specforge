 ...
 * so a section holding another one reports as itself and not as two halves.
 ...
 *
 * @param {string} html a template spec's HTML
 * @returns {{id:string|null, heading:string, level:number,
 *            subheadings:{text:string, level:number}[]}[]}
 */
export function parseTemplateOutline(html) {
  const out = [];
  for (const sec of findElements(String(html), SECTION_OPEN_RE, 'section')) {
    ...
    const body = removeElements(sec.html, PROMPT_OPEN_RE, 'div', 'before');
    const heads = [...body.matchAll(/<(h[1-6])\b[^>]*>([\s\S]*?)<\/\1>/gi)]
