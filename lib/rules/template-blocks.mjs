...
// and `cmdCreate` hands the prompts to the agent on the way past. The shell's
// stylesheet for prompts is stripped with them, for the reason given at
// `PROMPT_STYLE_RE`.
//
...
/**
 * The shell's stylesheet for prompts, which is scaffolding like the prompts are.
 *
 * A prompt has to look like guidance rather than like something the document
 * claims, and the rule that does that has to travel with the file: styled from
 * the review layer instead, a template exported or opened as a file shows its
 * guidance as ordinary prose. But a spec must not carry the string
 * `data-sf-prompt` at all, and a CSS selector is that string, so the block is
 * delimited and the strip takes it with the rest.
 *
 * Leading blank lines come with it, so a stripped spec has no gap where it was.
 */
const PROMPT_STYLE_RE =
  /(?:\n[ \t]*)+\/\*\s*specforge:prompt-style start[\s\S]*?specforge:prompt-style end\s*\*\//gi;

/**
 * That stylesheet. Drawn as an annotation ON the template rather than as
 * document furniture, so a reader never takes guidance for something the
 * document claims: dashed outline, legend chip, no fill, none of which any
 * component in the library uses.
 */
const PROMPT_STYLE = `  /* specforge:prompt-style start */
  /* Template guidance: what to write in the section it sits in. Rendered in
     with the prompts and stripped out with them, so no spec ever carries it. */
  [data-sf-prompt]{position:relative;margin:14px 0 24px;padding:16px 16px 14px;
    border:1px dashed var(--amber);border-radius:8px;background:transparent;
    color:var(--muted);font-size:13px;line-height:1.65}
  [data-sf-prompt]::before{content:"prompt";position:absolute;top:-8px;left:12px;
    padding:0 7px;background:var(--bg);color:var(--amber);
    font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:1.1px}
  [data-sf-prompt]>p{margin:0 0 8px}
  [data-sf-prompt]>p:last-child{margin:0}
 ...
 * takes the whitespace after it; a prompt is inserted on its own line inside a
 * section, so it takes the whitespace before it.
 *
...
export function stripTemplateBlocks(html) {
  ...
  const withoutPrompts = removeElements(withoutRules, PROMPT_OPEN_RE, 'div', 'before');
  return withoutPrompts.replace(PROMPT_STYLE_RE, '');
}
 ...
 *
 * Each prompt is inserted under the heading of the section it names, or at the
 * top of that section when it does not open with one. A prompt for a section the
 * shell does not have is skipped rather than dropped somewhere arbitrary.
 *
 * @param {string} shell
...
export function renderTemplateBlocks(shell, { rules = [], prompts = {} } = {}) {
  ...
  for (const [sectionId, text] of Object.entries(prompts)) {
    ...
    const body = text
      ...
      .join('\n');
    // Under the section's heading, not above it. Above the heading a prompt
    // renders between two sections, and a reader meets it as a trailing
    // paragraph of the one before: both shipped templates read that way, and it
    // was reported as prompts attached to the wrong section.
    //
    // Only a heading the section opens with counts. A heading further in belongs
    // to a subsection, and one nested inside a panel is the panel's, so a prompt
    // placed after either lands inside something it is not about. Those sections
    // keep the prompt at the top, which is where it has always been.
    const after = open.index + open[0].length;
    const heading = html.slice(after).match(/^\s*<(h[1-6])\b[^>]*>[\s\S]*?<\/\1\s*>/i);
    const at = after + (heading ? heading[0].length : 0);
    // Spliced rather than String.replace'd: a prompt is prose, and `$&` in it
    // pasted the matched section tag into the guidance.
    html = `${html.slice(0, at)}\n  <div data-sf-prompt>\n${body}\n  </div>${html.slice(at)}`;
  }

  // The stylesheet rides in with the prompts, and only when one landed: a shell
  // that took no guidance has nothing to style. It goes in the shell's own
  // <style> so it travels with the file, which a rule served by the review layer
  // does not: a template exported, or opened as a file, would show its guidance
  // as ordinary prose.
  if (inserted && html.includes('</style>')) {
    const at = html.indexOf('</style>');
    html = `${html.slice(0, at)}${PROMPT_STYLE}\n${html.slice(at)}`;
  }
