...
export const STRUCTURAL_RULES = [
  ...
  defineRule({
    ...
    severity: 'advisory',
    check: (html) => {
      // Only on specs that use one, so a clean report means something rather
      // than meaning "no disclosures here". Same shape as spec-components.
      const blocks = [...html.matchAll(/<details\b[^>]*>([\s\S]*?)<\/details>/gi)];
      if (!blocks.length) return { applies: false };
      const buried = [];
      for (const [, inner] of blocks) {
        for (const m of inner.matchAll(/<(h[23])\b/gi)) buried.push(m[1].toLowerCase());
      }
