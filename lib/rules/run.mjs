...
export function runCheckRules(rules, html) {
  ...
  for (const rule of rules) {
    ...
    try {
      ...
      // `throw null` and `throw undefined` are legal, and reading `.message` off
      // either throws again — inside the catch, where nothing is left to handle
      // it. That would take down the whole run, which is the exact failure this
