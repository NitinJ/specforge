...
export function defineRule(r) {
  ...
  // An id has to survive a round trip through `verify --judged a,b,c`. One
  // carrying a comma or a space could never be named there, so the gate would
  // fail on it forever with no way to settle it. Refused at authoring time,
  // where it is a typo, rather than discovered later as an unpassable spec.
  if (/[\s,]/.test(r.id)) {
    throw new Error(`rule ${JSON.stringify(r.id)}: an id cannot contain a comma or whitespace — it has to be nameable in --judged`);
  }
...
export function mergeRules(base, overrides) {
  ...
  for (const over of overrides) {
    ...
    if (!prev) {
      // Turning off a rule that is not there is a no-op, not an error. A
      // template carrying `data-sf-severity="off"` for an id that has since been
      // renamed or removed is stale, and the cost of being strict about it is
      // that every spec of that type stops verifying at all.
      if (over.severity === 'off') continue;
      // Otherwise a rule a template introduces is always prose; it has no way to
      // ship a function. defineRule refuses it if the sentence is missing.
