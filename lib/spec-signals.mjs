...
import { deliveryForSession, isConnected, isStale, watcherAlive } from './attach.mjs';
 ...
 *
 * A recent heartbeat alone is insufficient because it remains fresh after a
 * foreground Codex wait returns. The worker pid must still be live. Review
...
export function specConnected(id, meta) {
  ...
  return watcherAlive(m.attachedSession) && isConnected(m);
}
