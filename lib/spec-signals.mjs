...
import { isForAgent, wasSentToAgent, newestCommentAt } from './comments.mjs';
import { readMeta } from './meta.mjs';
import { deliveryForSession, isConnected, isStale } from './attach.mjs';
 ...
 *
 * All hosts require a live delivery worker and a fresh
 * heartbeat; a final beat alone cannot advertise an exited worker. Review
 * progress is shown by the action button and never promoted into a readiness
 * claim: an agent answering this round does not prove anything is listening for
 * the next one.
 */
export function specConnected(id, meta) {
  const m = meta || readMeta(id);
  if (!m || !m.attachedSession) return false;
  return deliveryForSession(m.attachedSession).state === 'ready' && isConnected(m);
}

/** The transport and readiness the browser may truthfully advertise. */
export function specDelivery(id, meta) {
  const m = meta || readMeta(id);
  if (!m || !m.attachedSession) {
    return { state: 'disconnected', mode: null, harness: null };
  }
  const delivery = deliveryForSession(m.attachedSession);
  if (delivery.state === 'ready' && !specConnected(id, m)) {
    return { ...delivery, state: 'disconnected' };
  }
  if ((delivery.state === 'paused' || delivery.state === 'disconnected')
      && reviewProgressForSpec(id) && !isStale(m)) {
    return { ...delivery, state: 'working' };
  }
  return delivery;
}
...
export function specSignals(id, shareInfo, meta) {
  ...
  return {
    ...
    // When the newest comment landed on this spec, in ms (0 for none): the
    // index orders collections by recency and a thread moving is activity as
    // much as an edit is (lib/collections.mjs).
    commented: newestCommentAt(threads),
