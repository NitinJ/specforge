 ...
 * session sits idle. So does a batch already in flight — the watcher stops the
 * moment it hands one over, and reporting "disconnected" while an agent is
 * visibly answering the previous round would be the same lie in reverse.
 *
 * That second half is bounded by the lock, or it would replace one false
 * positive with another: a session that died mid-round leaves its batch pending
 * forever, and the spec would claim to be connected to it indefinitely. Once
 * nothing has been heard from the session for STALE_MS, an unfinished round is
 * a leftover rather than work.
 */
export function specConnected(id, meta) {
  const m = meta || readMeta(id);
  if (!m || !m.attachedSession) return false;
  return isConnected(m) || (agentBusy(id) && !isStale(m));
}
