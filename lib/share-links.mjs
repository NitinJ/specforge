...
/**
 * Make a value safe inside double quotes WITHOUT re-encoding it.
 *
 * The suffix came out of the document, so it is already HTML: a query written
 * `?a=1&amp;b=2` is correct as it stands, and running it through escapeAttr
 * makes `&amp;amp;`, which the reader then follows with a literal "amp;" in it.
 * Only the characters that would end the attribute are touched.
 */
const requote = (s) => String(s).replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
 ...
 *
 * The lookahead rather than `\b`: there is a word boundary between the "a" and
 * the "-" of `<a-card>`, so `\b` claimed custom elements and rewrote their href
 * as though they were links. A tag name ends at whitespace, a slash or the
...
export function rewriteSpecLinks(html, { base = null, reachable = () => false } = {}) {
  return String(html).replace(OPEN_A, (tag, attrs, selfClose) => {
    ...
    if (base && reachable(target.id)) {
      ...
      // The path is constructed here and escaped; the suffix is the document's
      // own text and is only requoted.
      const to = `${escapeAttr(`${base}/spec/${target.id}`)}${requote(target.suffix)}`;
