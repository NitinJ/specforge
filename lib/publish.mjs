...
/**
 * One metrics request that cannot outlive `ms`, resolved to its parsed body.
 *
 ...
 *
 * The body is read inside the race rather than by the caller. Reading it later
 * would mean aborting the request first, and an aborted body makes a perfectly
 * good response unreadable, which reads as a failed poll every single time.
 *
 * @returns {Promise<object|null>} null when the response is missing or not ok
 */
async function fetchJsonWithDeadline(fetchImpl, url, ms, outerSignal) {
  ...
  try {
    ...
    clearTimeout(timer);
    ctl.abort(); // releases the request if the deadline won; a no-op once read
...
export async function publishViaCloudflared(port, opts = {}) {
  ...
  // Detached, so the tunnel is not in the daemon's process group and does not
  // go down with it. The daemon restarts on every server-side code change, and
  // a tunnel that died with it would take every URL already sent; the next
  // daemon adopts this process instead, by the pid recorded below.
  ...
  // Detaching keeps the child running past the parent; unref stops the child
  // from keeping the parent's event loop alive in the meantime.
  child.unref?.();
  ...
  // The pid is recorded so a daemon that was killed without running its
  // shutdown can find and reap the tunnel it left behind. `alive` answers the
  // question a share badge has to ask: cloudflared can die on its own, and a
  // publication whose tunnel is gone still looks published to anything that
  // only checks its own bookkeeping.
  return { url: `https://${hostname}`, pid: child.pid, stop, alive: () => !exited };
}
