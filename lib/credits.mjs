...
export function agentIdentity({ harness, model } = {}, env = process.env) {
  // resolveHarness drops a name it does not know and falls back to detection,
  // which would record a wrong credit that persists. Refuse it here instead.
...
export function recordCredit(id, role, agent) {
  ...
  if (role === 'author') {
    if (sameAgent(meta.author, who)) return meta;
    return writeMeta(id, { ...meta, author: who });
  }
  const next = withReviewer(meta.reviewers, who);
  return next ? writeMeta(id, { ...meta, reviewers: next }) : meta;
}
