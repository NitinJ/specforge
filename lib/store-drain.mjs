...
export const WATCH_CMD = REVIEW_WAIT_CMD;
 ...
 */
export function pendingWorkForSession(sessionId, env = process.env) {
  const reviews = pendingForSession(sessionId);
  if (reviews.length) {
    return {
      kind: 'review',
  ...
  const generates = generateRequestsForSession(sessionId);
  if (generates.length) {
    return {
      kind: 'generate',
      items: generates.map((m) => ({ specId: m.id, requestId: m.generate.requestedAt })),
      reason: generateReason(generates, env),
    };
  }
  const exports = exportRequestsForSession(sessionId);
  if (exports.length) {
    return {
      kind: 'export',
      items: exports.map((m) => ({ specId: m.id, requestId: m.export.requestedAt })),
...
function deliveryContinuation(env) {
  ...
  if (harness === CODEX_HARNESS) {
    return [
      'Then keep active review delivery running in this turn. Start this command in the',
      `foreground and leave its tool call active: ${REVIEW_WAIT_CMD}`,
    ];
  }
...
export function armWatcherReason(specIds, env = process.env) {
  ...
  return [
    ...
    codex
      ? 'Start active review delivery in the foreground now and leave its tool call active:'
      : 'Arm delivery in the background now:',
    `  ${codex ? REVIEW_WAIT_CMD : WATCH_CMD}`,
    '',
    `On completion it returns { ready, kind, work, reason }. Follow reason, then ${codex ? 'run it again' : 'relaunch it'}.`,
  ].join('\n');
}
