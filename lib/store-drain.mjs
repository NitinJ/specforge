...
// up by the session working the spec: its review watcher notices while the
// session is idle, and the turn_settled and turn_start events surface anything
// pending on the next turn, routing the agent to the review-spec skill. Which
// CLI that is, and what it calls those two moments, is lib/harness/.
//
// A batch on a spec whose session has gone waits for a human. There used to be a
// headless fallback that woke a fresh agent for those, off by default and never
 ...
 */
export function reviewReason(batches, harness = currentHarness()) {
  const review = harness.workRef('review-spec');
  ...
  return [
    ...
    ...lines,
    '',
    `Run ${review} now: for each batch, read its threads`,
 ...
 * reviewReason.
 */
export function armWatcherReason(specIds, harness = currentHarness()) {
  const review = harness.workRef('review-spec');
  return [
    ...
    `  ${WATCH_CMD}`,
    '',
    `On completion it returns { ready, pending } — on ready, run ${review}`,
    'for each pending spec and relaunch it.',
  ].join('\n');
}

/**
 * Instruction text for a session that has just started or resumed owning specs.
 *
 * The in-session watcher does not survive a restart and nothing re-arms it, so a
 * resumed session owning specs is deaf until told. Lives here rather than in the
 * hook because it is policy: what to say, not how to say it.
 */
export function sessionStartReason(specIds, harness = currentHarness()) {
  const review = harness.workRef('review-spec');
  return [
    `SpecForge: this session owns ${specIds.length} spec(s) under browser review. The`,
