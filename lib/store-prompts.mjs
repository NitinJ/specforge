...
export function sanitizePrompts(raw) {
  ...
  // Which of the two things `language` is. A store written before the Language
  // tab held the contract carries a short direction that was ADDED to the
  // shipped rules; one written since carries the whole contract, shipped rules
  // included. Showing the first as though it were the second would present two
  // sentences as the complete writing rules and lose the rest on the next save,
  // so the shape is stamped rather than guessed at from the text.
  if (language && raw.languageMode === 'contract') out.languageMode = 'contract';
...
export function writePrompts(patch) {
  ...
  // The mode travels with the text it describes: clearing one without the other
  // would leave a legacy direction stamped as a contract, or the reverse.
  if (raw.language === null) { delete next.language; delete next.languageMode; }
  else if (clean.language && !clean.languageMode) delete next.languageMode;
