...
export const MAX_LANGUAGE = 4000;
...
function sanitizeCustom(raw) {
  ...
  const out = {
    ...
    label,
    icon: str(raw.icon, 8) || '✦',
    kind: raw.kind === 'in-place' ? 'in-place' : 'aside',
    scope: raw.scope === 'global' ? 'global' : 'local',
    instruction,
  };
...
export function writePrompts(patch) {
  ...
  const next = { ...current, ...sanitizePrompts(patch) };
  const pa = sanitizePrompts(patch).actions;
  if (pa) next.actions = { ...(current.actions || {}), ...pa };
