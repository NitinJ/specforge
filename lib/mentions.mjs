...
import { agentNames } from './harness/index.mjs';

/** The one mention that routes a thread to an agent, whichever CLI answers it. */
 ...
 *
 * Every harness's agent name joins them, derived from the registry rather than
 * listed, so adding a CLI reserves its name with no second edit. A reply signed
 * `pi` must not be impersonable by a person who registered as `pi` first.
 *
 ...
 * write path can do.
 */
export const RESERVED_NAMES = new Set([AGENT_NAME, 'human', ...agentNames()]);
