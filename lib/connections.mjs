...
import { metaPath } from './store-paths.mjs';
...
export function setActive(specId, harnessId) {
  ...
  meta.attachedSession = conn.session;
...
export function beatConnection(specId, sessionKey, watcherPid) {
  ...
  // Straight to disk rather than through writeMeta, which stamps `updated` on
  // every call: a beat is not an edit to the document, and stamping it every 15s
  // pinned `updated` to the present for every attached spec.
