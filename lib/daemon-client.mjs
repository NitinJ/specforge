...
import { readServerState, healthOk } from './daemon-state.mjs';

const DAEMON = join(dirname(fileURLToPath(import.meta.url)), '..', 'server', 'daemon.mjs');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/**
 * True iff server.json advertises a daemon that actually answers /healthz.
 * Liveness is decided by the health endpoint alone — NOT by the recorded pid.
 * A recorded pid is unreliable: after an unclean exit the OS can reuse it, so
 * process.kill(pid, 0) reports "alive" for an unrelated process and we would
 * hand back a url with no daemon behind it. healthOk (a 1s-timeout GET) is the
 * only signal that a daemon is truly there; if it fails, ensureDaemon spawns a
 * fresh one (whose ensureServer reclaims any stale lock).
 */
export async function reusable() {
  const s = readServerState();
  if (s && (await healthOk(s.url))) return { url: s.url, port: s.port };
  return null;
}
