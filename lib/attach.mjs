...
import { promptSnippet } from './session-label.mjs';
 ...
 */
export function attach(specId, sessionId, cwd = process.cwd()) {
  ...
  if (owner && owner !== sessionId) {
    removeFromSession(owner, specId); // reclaim
    meta.sessionPrompt = null; // new owner → its own first prompt will be captured
  }
  meta.attachedSession = sessionId;
  meta.sessionCwd = cwd ? basename(cwd) : null; // friendly label: the project folder
...
/**
 * Record the owning session's first prompt as a friendly label, once per spec
 * (later prompts don't overwrite it). Called by the UserPromptSubmit hook.
 * @returns {number} how many specs were stamped
 */
export function recordFirstPrompt(sessionId, prompt) {
  const snip = promptSnippet(prompt);
  if (!snip) return 0;
  let n = 0;
  for (const id of specsForSession(sessionId)) {
    const meta = readMeta(id);
    if (meta && !meta.sessionPrompt) { writeMeta(id, { ...meta, sessionPrompt: snip }); n++; }
  }
  return n;
}
...
export function detach(specId) {
  ...
  meta.sessionCwd = null;
  meta.sessionPrompt = null;
