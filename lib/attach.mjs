...
export function attach(specId, sessionId) {
  ...
  const written = mutateMeta(specId, (fresh) => ({
    ...
    connections: withConnection(fresh, key),
  }));
...
export function detach(specId) {
  ...
  mutateMeta(specId, (meta) => {
    const next = { ...(meta.connections || {}) };
    if (owner) delete next[harnessOf(normalizeKey(owner))];
    return { ...meta, attachedSession: null, connections: next };
  });
...
function stamp(sessionId, field, { beatConnection = false } = {}) {
  ...
  const { watcherPid } = readSessionRecord(sessionId);
  for (const id of ids) {
    // Under a lock, and re-read inside it. A shared spec now has one writer per
    // connected harness, each beating every fifteen seconds, and an unlocked
    // read-modify-write of the whole file loses whichever update lands second.
    // What it loses is the other harness's connection record, which is the one
    // thing the switcher reads. It also protects an ordinary edit (a rename, a
    // project move) made between a beat's read and its write.
