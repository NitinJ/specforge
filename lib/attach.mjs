...
//
// SessionEnd releases clean exits. The owning session also bumps meta.heartbeat,
// and a lock older than STALE_MS remains reclaimable after a crash.
//
...
import { sessionsDir, sessionPath, metaPath, metaLockPath } from './store-paths.mjs';
import { sessionIdFromKey } from './harness-context.mjs';
...
/** The whole session record, including the active delivery worker lease. */
function readSessionRecord(sessionId) {
  try {
    const idx = JSON.parse(readFileSync(sessionPath(sessionId), 'utf8'));
    return {
      specs: Array.isArray(idx.specs) ? idx.specs : [],
      watcherPid: idx.watcherPid || null,
      worker: idx.worker && typeof idx.worker === 'object' ? idx.worker : null,
      harness: typeof idx.harness === 'string' ? idx.harness : null,
    };
  } catch {
    return { specs: [], watcherPid: null, worker: null, harness: null };
  }
}
...
export function clearWatcher(sessionId) {
  if (sessionId) writeSessionRecord(sessionId, { watcherPid: null, worker: null });
}

/**
 * Claim the one delivery slot for a session.
 *
 * The opaque lease prevents a worker that is shutting down late from clearing a
 * replacement worker. A live worker belonging to another process is refused;
 * a dead recorded pid is reclaimed after a crash.
 */
export function claimWorker(sessionId, {
  pid = process.pid,
  mode = 'background',
  harness = null,
  leaseId = randomUUID(),
  ...
  if (!sessionId) throw new Error('claimWorker: session id required');
  const current = readSessionRecord(sessionId);
  if (current.watcherPid && current.watcherPid !== pid && alive(current.watcherPid)) {
    throw new Error(`delivery worker already active for session ${sessionId}`);
  }
  const worker = { leaseId, pid, mode, harness: harness || current.harness, startedAt: now };
...
/** Clear a worker only when the caller still owns its lease. */
export function releaseWorker(sessionId, leaseId) {
  if (!sessionId || !leaseId) return false;
  const current = readSessionRecord(sessionId);
  if (!current.worker || current.worker.leaseId !== leaseId) return false;
  writeSessionRecord(sessionId, { watcherPid: null, worker: null });
  return true;
}

/** Current worker metadata, or null for legacy watcher-only records. */
export function workerFor(sessionId) {
  return readSessionRecord(sessionId).worker;
}

/** Remember which host owns a session independently of a short-lived worker. */
export function setSessionHarness(sessionId, harness) {
  if (sessionId && harness) writeSessionRecord(sessionId, { harness });
}

/** Browser-facing delivery state derived from a live process, not a stale beat. */
export function deliveryForSession(sessionId, alive = pidAlive) {
  if (!sessionId) return { state: 'disconnected', mode: null, harness: null };
  const record = readSessionRecord(sessionId);
  const harness = (record.worker && record.worker.harness) || record.harness || null;
  // Only the hook-owned Codex worker shares the daemon's host PID namespace.
  // Legacy tool-created worker records cannot establish readiness.
  if (harness === 'codex' && record.worker?.mode !== 'codex-queue') {
    return { state: 'disconnected', mode: null, harness };
  }
  const running = !!record.watcherPid && alive(record.watcherPid);
  if (running) {
    return {
      state: 'ready',
      mode: (record.worker && record.worker.mode) || 'native-idle',
      harness,
    };
  }
  return { state: 'disconnected', mode: null, harness };
...
export function watcherAlive(sessionId, alive = pidAlive) {
  const { watcherPid, worker, harness } = readSessionRecord(sessionId);
  if (((worker && worker.harness) || harness) === 'codex' && worker?.mode !== 'codex-queue') return false;
...
export function liveSessions(alive = pidAlive) {
  ...
  return entries
    ...
    .map((e) => sessionIdFromKey(e.name.slice(0, -'.json'.length)))
    .filter(Boolean)
    .filter((id) => watcherAlive(id, alive))
    .map((id) => {
      let at = 0;
      try { at = statSync(sessionPath(id)).mtimeMs; } catch { /* raced a delete */ }
      return { id, at };
    })
    .sort((a, b) => b.at - a.at)
    .map((s) => s.id);
...
export function attach(specId, sessionId) {
  ...
  // Under the lock, and re-read inside it. Taking ownership has no later beat to
  // restore it: a watcher writing a snapshot it read a moment earlier would hand
  // the spec straight back to whoever held it before.
  const written = mutateMeta(specId, (fresh) => ({
    ...fresh,
...
export function detach(specId) {
  const before = readMeta(specId);
  if (!before) return;
  const owner = before.attachedSession;
  // Under the lock for the same reason as attach: releasing ownership is not
  // self-healing, so a beat carrying a stale snapshot would silently re-attach.
  mutateMeta(specId, (meta) => ({ ...meta, attachedSession: null }));
  if (owner) {
    removeFromSession(owner, specId);
    if (specsForSession(owner).length === 0) clearWatcher(owner);
  }
}

/** Stop delivery on shutdown while retaining ownership for thread resume. */
export function endSession(sessionId, alive = pidAlive) {
  const count = specsForSession(sessionId).length;
  const current = readSessionRecord(sessionId);
  // SessionEnd can arrive after the same native thread has resumed. Preserve a
  // live replacement; a worker belonging to the ending process observes its
  // dead parent and releases its own lease within one poll.
  if (!current.watcherPid || !alive(current.watcherPid)) clearWatcher(sessionId);
  return count;
}
...
function stamp(sessionId, field) {
  ...
  const now = Date.now();
  for (const id of ids) {
    // Under a lock, and re-read inside it. A spec two sessions share has one
    // writer per session, each beating every fifteen seconds, and an unlocked
    // read-modify-write of the whole file loses whichever update lands second.
    // What it loses is not this field but everything else the other writer set.
    // It also protects an ordinary edit (a rename, a project move) made between
    // a beat's read and its write.
    withFileLock(metaLockPath(id), () => {
      const meta = readMeta(id);
