...
export function deliveryForSession(sessionId, alive = pidAlive) {
  ...
  // Legacy Codex worker PIDs belong to a tool sandbox, not this process's
  // namespace. Codex now uses hook delivery and never advertises a live watcher.
  if (harness === 'codex') return { state: 'paused', mode: 'next-turn', harness };
...
export function watcherAlive(sessionId, alive = pidAlive) {
  ...
  if (((worker && worker.harness) || harness) === 'codex') return false;
