...
function renderBlockInner(n, ctx, depth = 0) {
  ...
  switch (tag) {
    ...
    case 'details': {
      ...
      // One line, and no angle brackets. `<summary>` is a raw HTML tag here, so
      // two things inside it end it early: a newline, which ends the tag as far
      // as a markdown renderer is concerned, and a literal `</summary>` — which
      // a summary can genuinely produce, because `<code>&lt;/summary&gt;</code>`
      // decodes on the way through. Either one spills the rest of the summary
      // into the document as loose text.
      // EVERY angle bracket is escaped, and that is what makes a tag readable
      // rather than what hides it.
      //
      // `<details>` opens a markdown HTML block, which runs until a blank line,
      // and markdown is not parsed inside one. So the summary line is raw HTML
      // however it is written: a code span there stays literal backticks, and a
      // bare `<div>` is a tag the renderer acts on or drops. The only way to
      // show a reader the characters `<div>` is to emit `&lt;div&gt;`, which raw
      // HTML renders as that text.
      //
      // An earlier pass narrowed this to `</summary` alone on the reasoning that
      // it is the only sequence that can end the tag. True, and beside the
      // point: the others do not end the tag, they disappear into it.
      // The `\<` undo comes first. `inline()` markdown-escapes a tag-shaped `<`
      // so GFM will not read it as raw HTML, which is right everywhere except
      // here: inside an HTML block no backslash escape means anything, so
      // escaping the result again left a visible `\&lt;div&gt;` on the page.
      const summary = sum
        ? inline(sum[1], ctx).replace(/\s+/g, ' ').trim()
          .replace(/\\</g, '<')
          .replace(/</g, '&lt;').replace(/>/g, '&gt;')
        : '';
