...
function styleRules(html) {
  ...
  while (i < css.length) {
    ...
    if (open === -1) break;
    // `@charset` and `@import` end at a semicolon, not a block. Everything up to
    // the last one belongs to them, and only what follows is the selector; the
    // whole run would otherwise read as one at-rule and take the next rule with it.
    const sel = css.slice(i, open).split(';').pop().trim();
    let depth = 1;
    let j = open + 1;
