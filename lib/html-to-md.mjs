...
function renderBlockInner(n, ctx, depth = 0) {
  ...
  switch (tag) {
    ...
    case 'details': {
      ...
      // Collapsed to one line. `<summary>` is a raw HTML tag here, and a newline
      // inside it ends the tag as far as a markdown renderer is concerned, which
      // spills the rest of the summary into the document as literal text.
      const summary = sum ? inline(sum[1], ctx).replace(/\s+/g, ' ').trim() : '';
      const rest = sum ? inner.slice(inner.indexOf(sum[0]) + sum[0].length) : inner;
      const body = renderChildren(rest, ctx, depth + 1);
      // The `open` ATTRIBUTE, not the word. `\bopen\b` also matches
      // `class="open-question"`, which would export a collapsed disclosure as an
      // expanded one and silently change what the document says on first read.
      const open = /(?:^|\s)open(?=[\s=/>]|$)/i.test(attrs || '') ? '<details open>' : '<details>';
