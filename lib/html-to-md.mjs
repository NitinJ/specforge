...
/**
 * The inline markup a `<summary>` may carry into the export.
 *
 * Formatting only, and it is the complete set: a summary is one line of label.
 */
const SUMMARY_TAGS = new Set([
  'code', 'strong', 'b', 'em', 'i', 'u', 's', 'del', 'ins', 'mark',
  'kbd', 'samp', 'var', 'abbr', 'cite', 'q', 'small', 'sub', 'sup', 'span', 'br', 'wbr',
]);

/**
 * A summary's markup, reduced to what may be published.
 *
 * The summary is emitted as raw HTML rather than converted to markdown (see the
 * `details` case), which is what makes `<code>` render as code. That also makes
 * it the one place author markup reaches a reader's renderer verbatim, and specs
 * are imported from untrusted markdown and then published to people who are not
 * the author: a `<script>` or an `onclick` in a summary would run for them.
 *
 * So every tag outside the formatting set is escaped to its own text, and every
 * attribute comes off the ones that stay — no `href`, no `style`, and no event
 * handler can survive an allow-list that keeps no attributes at all.
 */
function safeSummary(html) {
  return String(html).replace(/<\/?([a-zA-Z][\w-]*)((?:"[^"]*"|'[^']*'|[^>])*)>/g, (tag, name) => {
    const t = name.toLowerCase();
    if (!SUMMARY_TAGS.has(t)) return tag.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    return tag.startsWith('</') ? `</${t}>` : `<${t}>`;
  });
}
...
/**
 * A task item split into its text and its verify note.
 *
 * The note has been written two ways. Older plans trail a `<span class="verify">`
 * inside the task; the impl template nests a list holding an `<li class="verify">`.
 * Both are lifted out, because a note left in place folds into the task line and
 * the task reads "Write it. verify: it runs".
 */
function taskParts(html, ctx) {
  const flat = (s) => inline(s, ctx).replace(/\s+/g, ' ').trim();
  let body = html;
  let verify = '';
  for (const el of nodes(html)) {
    if (el.kind !== 'el') continue;
    if (classes(el.attrs).has('verify')) {
      verify = flat(el.inner);
      body = body.replace(new RegExp(`<${el.tag}\\b[^>]*class="[^"]*\\bverify\\b[^"]*"[\\s\\S]*?</${el.tag}>`, 'i'), '');
      continue;
    }
    // A wrapper list whose only content is the note: the note goes, and so does
    // the empty list left behind.
    if (el.tag === 'ul' || el.tag === 'ol') {
      const items = [...nodes(el.inner)].filter((x) => x.kind === 'el' && x.tag === 'li');
      if (!items.length || !items.every((x) => classes(x.attrs).has('verify'))) continue;
      verify = items.map((x) => flat(x.inner)).join(' ');
      body = body.replace(`<${el.tag}${el.attrs}>${el.inner}</${el.tag}>`, '');
    }
  }
  return { text: flat(body), verify };
}
...
function renderPlan(inner, ctx) {
  ...
  for (const stage of nodes(inner)) {
    ...
    for (const el of nodes(stage.inner)) {
      if (el.kind !== 'el') continue;
      // `ol` as well as `ul`: the impl template numbers its tasks. The list tag
      // is presentation and markdown carries a checkbox list either way.
      if ((el.tag === 'ul' || el.tag === 'ol') && classes(el.attrs).has('sf-tasks')) {
        for (const t of nodes(el.inner)) {
          ...
          const { text, verify } = taskParts(t.inner, ctx);
...
function renderBlockInner(n, ctx, depth = 0) {
  ...
  switch (tag) {
    ...
    case 'div': {
      ...
      // Tabs flatten to their panels in order, each under a BOLD LABEL rather
      // than a heading (D8). A tab label is an h5 by the library's own
      // definition — a label over a run of prose, not a division of the document
      // — and exporting four panels as four headings would put four entries into
      // the outline of the exported markdown that are not sections of it.
      if (cls.has('tabs')) {
        const out = [];
        for (const child of nodes(inner)) {
          if (child.kind !== 'el' || !classes(child.attrs).has('tab')) continue;
          const label = plainText(attr(child.attrs, 'data-label'));
          if (label) out.push(`**${escapeText(label)}**`);
          out.push(...renderChildren(child.inner, ctx, depth + 1));
        }
        return out;
      }
    ...
    // Passed through as HTML rather than flattened to a heading and prose.
    // GitHub, GitLab and every renderer that allows raw HTML render it as the
    // same collapsible block, so the export keeps the property the component was
    // chosen for; flattening would turn "detail on a second pass" into detail on
    // the first, which is the whole distinction.
    //
    // The blank lines are load-bearing. Without one after </summary> the body is
    // inside a raw HTML block, and a table or a list in there renders literally
    // (pipes and hyphens) instead of as markup.
    case 'details': {
      const sum = inner.match(/<summary\b[^>]*>([\s\S]*?)<\/summary>/i);
      // The summary's own HTML, unchanged except for collapsing it onto one
      // line. NOT converted to markdown.
      //
      // `<details>` opens a markdown HTML block, which runs until a blank line,
      // and markdown is not parsed inside one — but HTML is exactly what a raw
      // HTML block renders. The author's markup is therefore already the right
      // answer: `<code>x</code>` renders as code, `<strong>` as bold, and
      // `&lt;div&gt;` as the text `<div>`.
      //
      // Converting it to markdown first put literal backticks and asterisks on
      // the page, and three rounds of repair after that (escape the angle
      // brackets; no, only `</summary`; no, and undo the markdown escape the
      // escaping had exposed) were all chasing a conversion that should not have
      // happened. A literal `</summary>` cannot appear here either: it would
      // have ended the tag in the source too.
      //
      // One line, because a newline ends the tag as far as a markdown renderer
      // is concerned and spills the rest of the summary into the document.
      const summary = sum ? safeSummary(sum[1].replace(/\s+/g, ' ').trim()) : '';
      const rest = sum ? inner.slice(inner.indexOf(sum[0]) + sum[0].length) : inner;
      const body = renderChildren(rest, ctx, depth + 1);
      // The `open` ATTRIBUTE, not the word anywhere in the tag. Quoted values are
      // stripped first: `\bopen\b` matched `class="open-question"`, and even
      // anchored to a word boundary it still matched the `open` inside
      // `class="foo open bar"` or `title="open questions"`. Exporting a
      // collapsed disclosure as an expanded one silently changes what the
      // document says on first read.
      // The `="…"` goes, the attribute NAME stays. Stripping the whole quoted
      // run took `open=""` with it — valid boolean-attribute syntax — and
      // exported an expanded disclosure as collapsed, which is the same defect
      // this guard was added to prevent, in the other direction.
      // Unquoted values go too (`data-x=open` is legal HTML and would otherwise
      // read as the attribute), and the name always survives, so `open=""` and
      // `open="open"` are still recognised.
      const bareAttrs = String(attrs || '')
        .replace(/=\s*"[^"]*"|=\s*'[^']*'|=\s*[^\s"'>]+/g, '');
      const open = /(?:^|\s)open(?=[\s=/>]|$)/i.test(bareAttrs) ? '<details open>' : '<details>';
      return [open, `<summary>${summary}</summary>`, '', ...body, '', '</details>'];
    }
    // Reached only for a <summary> outside a <details>, which is malformed. Its
    // text is still the author's, so it is kept rather than dropped.
    case 'summary':
      return renderChildren(inner, ctx, depth);
