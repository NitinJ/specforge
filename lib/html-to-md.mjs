...
function renderPlan(inner, ctx) {
  ...
  for (const stage of nodes(inner)) {
    ...
    if (!derivable) out.push(`<!-- sf:stage id="${id}"${pr ? ` pr="${pr}"` : ''} -->`);
    out.push('');
      ...
      if (el.tag === 'ul' && classes(el.attrs).has('sf-tasks')) {
        for (const t of nodes(el.inner)) {
          ...
          if (status !== 'done' && status !== 'todo') {
            tasks.push(`      <!-- sf:task id="${tid}" status="${status}" -->`);
          }
...
function liftSvg(el, label, ctx) {
  ...
  const name = `${ctx.sectionId}-${ctx.sectionAssets}.svg`;
...
function renderBlock(n, ctx, depth = 0) {
  ...
  switch (tag) {
    ...
    case 'p': {
      const t = inline(inner, ctx).replace(/[ \t]+/g, ' ').trim();
      return t ? [t] : [];
    }
    case 'ul': case 'ol': {
    ...
    case 'div': {
      if (cls.has('callout')) {
        const variant = ['warn', 'good', 'bad'].find((v) => cls.has(v)) || '';
...
function renderChildren(html, ctx, depth = 0) {
  const out = [];
  for (const n of nodes(html)) out.push(...renderBlock(n, ctx, depth));
...
export function specToMarkdown(html, opts = {}) {
  ...
  for (const id of getSectionIds(html)) {
    ...
    if (headingSlug(headingText) !== id) blocks.push(`<!-- sf:section id="${id}" -->`);
