...
function renderBlockInner(n, ctx, depth = 0) {
  ...
  switch (tag) {
    ...
    case 'details': {
      ...
      // Only `</summary` is neutralised, because it is the only sequence that
      // can end the tag. Escaping every angle bracket also worked, and changed
      // what a reader sees: a summary mentioning `<div>` in a code span rendered
      // the entities instead of the tag.
      const summary = sum
