// Tolerant parsing + structural checks for SpecForge spec HTML.
//
// SpecForge owns the spec format, so lightweight regex parsing is sufficient and
// keeps the plugin dependency-free. Specs do not nest <section> elements, which
// makes section extraction unambiguous.

import { PALETTE_TOKENS } from './config.mjs';

/** Read an HTML attribute value out of an element's attribute string. */
export function getAttr(attrs, name) {
  const m = attrs.match(new RegExp(`\\b${name}="([^"]*)"`));
  return m ? m[1] : null;
}

/** All `<section id="…">` ids, in document order (may contain duplicates). */
export function getSectionIds(html) {
  const ids = [];
  const re = /<section\b[^>]*\bid="([^"]+)"/g;
  let m;
  while ((m = re.exec(html))) ids.push(m[1]);
  return ids;
}

/**
 * Ids of sections carrying `data-sf-aside`: agent drafts attached to the section
 * above them, not part of the document's outline.
 *
 * Attribute-order tolerant, because the attribute may be written before or after
 * the id.
 */
export function getAsideSectionIds(html) {
  const ids = [];
  const re = /<section\b([^>]*)>/g;
  let m;
  while ((m = re.exec(html))) {
    if (!/\bdata-sf-aside\b/.test(m[1])) continue;
    const id = getAttr(m[1], 'id');
    if (id) ids.push(id);
  }
  return ids;
}

/**
 * The attribute string of `<section id="…">`, or null.
 *
 * Attribute-order tolerant, which the id-first regex above is not: a caller that
 * needs `data-sf-action` off a section cannot assume where it was written.
 */
export function sectionAttrs(html, id) {
  const re = /<section\b([^>]*)>/g;
  let m;
  while ((m = re.exec(html))) if (getAttr(m[1], 'id') === id) return m[1];
  return null;
}

/** Section ids that appear more than once. */
export function duplicateSectionIds(html) {
  const seen = new Set();
  const dups = new Set();
  for (const id of getSectionIds(html)) {
    if (seen.has(id)) dups.add(id);
    seen.add(id);
  }
  return [...dups];
}

/** Inner HTML of a `<section id="…">…</section>` (first match), or null. */
export function sectionBody(html, id) {
  const m = html.match(
    new RegExp(`<section\\b[^>]*\\bid="${id}"[^>]*>([\\s\\S]*?)</section>`)
  );
  return m ? m[1] : null;
}

/**
 * Which of `required` section ids are present / missing.
 * @returns {{present:string[], missing:string[]}}
 */
export function requiredSectionStatus(html, required) {
  const have = new Set(getSectionIds(html));
  const present = [];
  const missing = [];
  for (const id of required) (have.has(id) ? present : missing).push(id);
  return { present, missing };
}

/**
 * Check the light/dark theme contract every spec must satisfy. The spec ships
 * the theme CSS (light/dark vars, a [data-theme] override, prefers-color-scheme);
 * applying + persisting the choice is the review layer's job, so there's no
 * in-spec toggle / localStorage requirement.
 * @returns {{ok:boolean, missing:string[]}}
 */
export function checkThemeContract(html) {
  const checks = {
    'css-variables': /:root\s*\{[\s\S]*?--bg\s*:/.test(html),
    'light-override': /\[data-theme=["']light["']\]/.test(html) || html.includes('data-theme="light"'),
    'prefers-color-scheme': html.includes('prefers-color-scheme'),
  };
  const missing = Object.entries(checks).filter(([, ok]) => !ok).map(([k]) => k);
  return { ok: missing.length === 0, missing };
}

/**
 * Check the spec defines the canonical palette tokens (a `--<token>:` definition
 * for each). Keeping every spec on one token set is what lets the review-layer
 * theme variants re-tint reliably instead of guessing per-spec dialects.
 * @param {string[]} [tokens] bare token names (no leading `--`)
 * @returns {{ok:boolean, missing:string[]}} missing tokens (bare names)
 */
export function checkPalette(html, tokens = PALETTE_TOKENS) {
  const missing = tokens.filter((t) => !new RegExp('--' + t + '\\s*:').test(html));
  return { ok: missing.length === 0, missing };
}

/**
 * How many of a row's progress steps are complete.
 *
 * Absent on every spec written before the trackers grew a progress bar, and
 * those have to read as zero: a cell rendered from `undefined` says NaN.
 */
function stepsOf(attrs) {
  const n = Number.parseInt(getAttr(attrs, 'data-sf-steps') ?? '', 10);
  return Number.isFinite(n) && n > 0 ? n : 0;
}

/**
 * Parse the structured implementation plan into stages and tasks.
 * @returns {{stage:string, pr:string|null, steps:number,
 *   tasks:{id:string, status:string, steps:number}[]}[]}
 */
export function parsePlan(html) {
  const plan = sectionBody(html, 'impl-plan');
  if (!plan) return [];
  const stages = [];
  let current = null;
  const tagRe = /<li\b([^>]*\bdata-sf-(?:stage|task)="[^"]*"[^>]*)>/g;
  let m;
  while ((m = tagRe.exec(plan))) {
    const attrs = m[1];
    const stage = getAttr(attrs, 'data-sf-stage');
    if (stage !== null) {
      current = { stage, pr: getAttr(attrs, 'data-sf-pr'), steps: stepsOf(attrs), tasks: [] };
      stages.push(current);
      continue;
    }
    const task = getAttr(attrs, 'data-sf-task');
    if (task !== null) {
      const entry = {
        id: task,
        status: getAttr(attrs, 'data-sf-status') || 'todo',
        steps: stepsOf(attrs),
      };
      if (current) current.tasks.push(entry);
      else stages.push({ stage: null, pr: null, steps: 0, tasks: [entry] });
    }
  }
  return stages;
}

/** True if the plan has at least one stage and one task with a status. */
export function hasStructuredPlan(html) {
  const plan = parsePlan(html);
  return plan.some((s) => s.tasks.length > 0);
}

/** Strip HTML tags and collapse whitespace. */
function stripTags(s) {
  return s.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
}

/**
 * Turn HTML text back into the text it came from.
 *
 * The inverse of escHtml below, and it has to exist: a title read out of a
 * document is stored in meta.json and then escaped again by every renderer, so
 * without this an `&` in a title reaches the index as a literal `&amp;`.
 *
 * `&amp;` is decoded last on purpose. Decoded first, `&amp;lt;` would become
 * `&lt;` and then `<`, which is a title nobody wrote.
 */
function decodeEntities(s) {
  return s
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"')
    .replace(/&(?:apos|#0*39);/g, "'")
    .replace(/&nbsp;/g, ' ')
    .replace(/&amp;/g, '&');
}

/** The spec's display title (from <title>, with a trailing " — Spec" removed). */
export function getTitle(html) {
  const m = html.match(/<title>([\s\S]*?)<\/title>/i);
  if (!m) return 'Untitled spec';
  const text = decodeEntities(stripTags(m[1]));
  return text.replace(/\s*[—·-]\s*Spec\s*$/i, '').trim() || 'Untitled spec';
}

/** Escape text for safe interpolation into HTML element content. */
function escHtml(s) {
  return String(s).replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
}

/**
 * Rewrite the spec's display title in place: the first `<h1>` content and the
 * `<title>` element. Returns the new HTML (unchanged if neither is present).
 * Used by rename so the document heading tracks the listing name.
 */
export function setTitle(html, title) {
  const safe = escHtml(title);
  // Replacer FUNCTIONS, not a string — so `$` in the title (e.g. "Cost $2/mo")
  // is never interpreted as a $1/$&/$' replacement-pattern token.
  return html
    .replace(/(<title>)[\s\S]*?(<\/title>)/i, (_m, open, close) => open + safe + close)
    .replace(/(<h1\b[^>]*>)[\s\S]*?(<\/h1>)/i, (_m, open, close) => open + safe + close);
}

/**
 * The spec's lifecycle status. Prefers an explicit `data-sf-spec-status` on the
 * root element, then the header status badge, defaulting to "draft".
 */
export function getStatus(html) {
  const attr = html.match(/<html\b[^>]*\bdata-sf-spec-status="([^"]+)"/i);
  if (attr) return attr[1].trim();
  const badge = html.match(/status:\s*<span[^>]*>([\s\S]*?)<\/span>/i);
  if (badge) return stripTags(badge[1]);
  return 'draft';
}
