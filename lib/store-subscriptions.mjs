 ...
 *
 * Staged in two calls on purpose. Serialising to the temp file is the slow
 * part; `commit` is a single rename. Callers therefore do all their work, then
 * check they still hold the lock, then commit — which leaves nothing but the
 * rename between the ownership check and the write.
 */
function stageAll(subs) {
  mkdirSync(storeRoot(), { recursive: true });
  const tmp = `${subscriptionsPath()}.${process.pid}.${randomUUID()}.tmp`;
  writeFileSync(tmp, JSON.stringify(subs, null, 2));
  return () => renameSync(tmp, subscriptionsPath());
}

/**
 * How long a lock may go untouched before another process reclaims it.
 *
 * Generous, because nothing waits on this lock: no daemon request handler
 * blocks behind it, only a person's `join` or `leave`. The critical section is
 * synchronous and sub-millisecond, so exceeding this needs the OS to suspend
 * the process outright; a longer window costs one CLI a slower recovery from a
 * genuinely crashed holder and buys a wide margin against reclaiming a live one.
 */
 ...
 * comments.json uses (lib/store-comments.mjs), with two deliberate differences.
 *
 * **It refuses rather than degrading.** `withCommentsLock` proceeds unlocked
 * past its wait budget because its caller is a request handler inside the
 * daemon, where hanging or failing a reviewer's comment is worse than a rare
 * lost update. Nothing here has that shape: join and leave are one-shot CLI
 * commands, so refusing costs the user one re-run and never silently drops a
 * subscription someone else just added.
 *
 * **The lock is owned, not merely held.** Stale reclamation is what makes a
 * crashed holder recoverable, and it is also what lets a holder that merely
 * *paused* past the staleness window find its lock reassigned. So the file
 * carries a per-acquisition id: the holder releases only a lock still bearing
 * its own id, and `fn` is handed an `owned()` it calls immediately before
 * writing. A holder that lost the lock therefore neither deletes a stranger's
 * lock nor commits a snapshot taken before the reassignment.
 */
function withSubscriptionsLock(fn) {
  ...
  for (;;) {
    try {
      ...
      try {
        // Reclaim is conditional on the lock still being the one observed as
        // stale. An unconditional delete here would remove a lock a new holder
        // acquired between the stat and the rm, which is how a reclaimer breaks
        // a perfectly healthy critical section.
        const held = readFileSync(path, 'utf8');
        if (Date.now() - statSync(path).mtimeMs > LOCK_STALE_MS) {
          if (readFileSync(path, 'utf8') === held) rmSync(path, { force: true });
          continue;
        }
      } catch { continue; }
  ...
  /** Whether this call still holds the lock it acquired. */
  const owned = () => {
    try {
      return readFileSync(path, 'utf8') === id;
    } catch {
      return false;
    }
  };
  try {
    ...
    // Only ours to remove. A lock reclaimed as stale now belongs to whoever
    // took it, and deleting theirs would let a third process in alongside them.
    if (owned()) {
...
/** Throw unless this call still owns the lock; used right before every write. */
function assertOwned(owned) {
  if (!owned()) {
    throw new Error('subscriptions.json lock was reclaimed mid-update; try again');
  }
}
...
export function addSubscription({ name, origin, token }) {
  ...
  return withSubscriptionsLock((owned) => {
...
export function removeSubscription(key) {
  return withSubscriptionsLock((owned) => {
    ...
    const commit = stageAll(subs.filter((s) => s !== matches[0]));
    assertOwned(owned);
    commit();
    return true;
  });
