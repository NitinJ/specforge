...
import { specTypes, specType, shadowedTypes } from './spec-types.mjs';

const records = specTypes().map((slug) => specType(slug));
const shadowed = shadowedTypes();

if (process.argv.includes('--json')) {
  process.stdout.write(`${JSON.stringify({ types: records, shadowed }, null, 2)}\n`);
} else {
  ...
  // Rows the plugin has since taken over. Said here rather than nowhere: the
  // description their author wrote has been replaced by the shipped one, and a
  // reader comparing this list against types.json would otherwise find entries
  // that do not appear and no reason why.
  if (shadowed.length) {
    lines.push('');
    lines.push(`${shadowed.length} type${shadowed.length === 1 ? '' : 's'} in types.json now ship`);
    lines.push('with SpecForge, so the shipped definition is used and the stored row is');
    lines.push('ignored. Their template specs are untouched and still yours to edit.');
    for (const t of shadowed) lines.push(`  ${t.slug}`);
  }
