...
export function generateReason(metas, env = process.env) {
  const lines = metas.map((m) => `  - ${m.id} (kind "${m.type}")\n      prompt: ${m.generate.prompt}`);
  return [
    `SpecForge: ${metas.length} template(s) waiting to be written:`,
    ...lines,
    '',
    `Run the ${skillRef('generate-template', env)} skill now for each: turn the prompt into`,
    'the template spec\'s HTML. Start with specforge template-working <id>, lint it,',
    'then report back with specforge template-done <id> (or --error "<msg>" if it cannot be written).',
