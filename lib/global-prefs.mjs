...
// full variant.) It also holds the project list and which project is selected,
// which are the index page's alone — spec pages take theme and font by name,
// never the whole object, because a published spec is served the same review
// layer a local one is and how the author has organised their store is nobody's
// business but theirs.
...
/** How many project names the stored order will hold. */
const MAX_ORDER = 200;
...
/**
 * The order the index lists projects in, as names.
 *
 * A project is a name before it holds anything — it can exist while empty, so
 * there is nowhere else its rank could live. A name that no longer matches any
 * spec is kept rather than pruned: moving the last spec out of a project and
 * back does not lose its place. (Collections need no such list: they order
 * themselves by recency — lib/collections.mjs.)
 */
