...
import { renameSpec, deleteSpec, readSpecHtml, writeSpecHtml } from './store.mjs';
import { getSectionInner, replaceSectionInner } from './section-edit.mjs';
...
/**
 * GET /api/spec/:id/section/:sid — the CLEAN inner html of `<section id="sid">`
 * read from disk (not the chrome-polluted served DOM), for the inline editor.
 */
export function handleSectionGet(id, sectionId, res) {
  if (!specOr404(id, res)) return;
  const inner = getSectionInner(readSpecHtml(id), sectionId);
  if (inner == null) return sendJson(res, 404, { error: 'section not found' });
  sendJson(res, 200, { specId: id, sectionId, html: inner });
}

/**
 * PUT /api/spec/:id/section/:sid — replace that section's inner html on disk,
 * keeping the <section> wrapper (id + classes) untouched. The write triggers the
 * live reload. Not blocked for template specs: editing a template's content IS
 * the template-editing flow.
 */
export function handleSectionPut(id, sectionId, body, res) {
  if (!specOr404(id, res)) return;
  if (typeof (body && body.html) !== 'string') {
    return sendJson(res, 400, { error: 'html (string) required' });
  }
  const next = replaceSectionInner(readSpecHtml(id), sectionId, body.html);
  if (next == null) return sendJson(res, 404, { error: 'section not found' });
  writeSpecHtml(id, next);
