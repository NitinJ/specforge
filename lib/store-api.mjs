...
export function handleMeta(id, res, isLive) {
  const meta = readMeta(id);
  ...
  sendJson(res, 200, {
    ...
    // The public URL this spec is on right now, or null. Read from the store,
    // not from the publications registry, so it is the same answer everywhere
    // and survives the page being served by a publication listener.
    share: shareOf(id, isLive),
  });
}

/**
 * The publication this spec has, and whether it actually works.
 *
 * `live` is the part that matters. A record on disk is not proof the link
 * serves: a daemon killed outright leaves the record and the tunnel process
 * behind while the listener they point at is gone, so the URL answers 502. Only
 * the running daemon's registry knows, which is why it is passed in.
 *
 * @param {string} id
 * @param {(id:string) => boolean} [isLive] from the daemon's publications registry
 * @returns {{url:string, createdAt:string, live:boolean}|null}
 */
function shareOf(id, isLive) {
  const rec = readShare(id);
  if (!rec) return null;
  return { url: rec.url, createdAt: rec.createdAt, live: isLive ? !!isLive(id) : false };
}
