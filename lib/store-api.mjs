...
import { connectionList, activeHarnessOf, setActive } from './connections.mjs';
...
export function handleMeta(id, res, shareInfo) {
  ...
  sendJson(res, 200, {
    ...
    // Every harness connected to this spec, which one is working it, and whether
    // each is alive. What the header switcher renders (P9, P10). Owner-only: a
    // reviewer holding a published link has no agent to switch to.
    harnesses: connectionList(meta),
    activeHarness: activeHarnessOf(meta),
...
export function handleOrganize(id, body, res) {
  if (!specOr404(id, res)) return;
  if (!notTemplateOr403(id, res)) return;
  // Validated before the lock is taken, so a rejected body never opens one.
  let coll;
...
/**
 * POST /api/spec/:id/active — choose which connected harness works this spec.
 *
 * The only writer of the active harness, and it is reached only from the
 * browser, which is a person (I7b). No CLI verb sets it, which is what makes E8
 * true: nothing an agent does can take work from another, or strand a spec by
 * crashing while it holds it.
 */
export function handleSetActive(id, body, res) {
  if (!specOr404(id, res)) return;
  const harness = body && typeof body.harness === 'string' ? body.harness.trim() : '';
  if (!harness) return sendJson(res, 400, { error: 'harness required' });
  const out = setActive(id, harness);
  if (!out.ok) return sendJson(res, 409, { error: out.error });
  const meta = readMeta(id);
  sendJson(res, 200, {
    ok: true,
    id,
    activeHarness: out.active,
    changed: out.changed,
    harnesses: connectionList(meta),
    sessionLabel: sessionDisplay(meta),
  });
