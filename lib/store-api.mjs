...
export function handleMeta(id, res) {
  ...
  sendJson(res, 200, {
    ...
    // Whether this spec is on a public URL right now. Read from the store, not
    // from the publications registry, so it is the same answer everywhere and
    // survives the page being served by a publication listener.
    shared: !!readShare(id),
  });
}
