...
import { defaultMeta, readMeta, writeMeta, mutateMeta } from './meta.mjs';
...
export function renameSpec(id, title) {
  ...
  // Through the lock, which also re-reads: writeSpecHtml just bumped `updated`,
  // and a watcher beat may have written between the two.
  return mutateMeta(id, (m) => ({ ...m, title }));
}
...
export function writeSpecHtml(id, html) {
  mkdirSync(specDir(id), { recursive: true });
  writeFileSync(specHtmlPath(id), html);
  // Under the lock so the touch cannot roll back a concurrent connection or
  // ownership change: it re-reads and writes the whole file, same as any other
  // read-modify-write here.
  mutateMeta(id, (m) => m); // mutateMeta -> writeMeta bumps `updated`
}
