...
//
// Agent replies are excluded (author !== 'human'); only what the human wrote,
// because the point is to learn what they keep asking for.
//
// Usage:
...
export function mineComments() {
  ...
  for (const e of entries) {
    ...
    const threads = (store.threads || [])
      .map((t) => {
        const human = (t.comments || []).filter((c) => c.author === 'human');
        total += human.length;
  ...
  for (const s of specs) {
    ...
    for (const t of s.threads) {
      for (const body of t.comments) {
        console.log(`  · [${t.section}] ${body.replace(/\s+/g, ' ').slice(0, 200)}`);
      }
    }
  }
}
