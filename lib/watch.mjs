 ...
 * SPECFORGE_WATCH_CLAUDE_ARGS.
 */
export function spawnClaudeDrain(specsDir, projectDir, pending, log = () => {}) {
  return new Promise((resolve) => {
    const reason = reviewReason(specsDir, pending);
    const extra = (process.env.SPECFORGE_WATCH_CLAUDE_ARGS || '').split(' ').filter(Boolean);
    let child;
    try {
...
export function createWatcher({ specsDir, projectDir = specsDir, intervalMs = 90000, drain, log = () => {} }) {
  const drainFn = drain || ((sd, pending) => spawnClaudeDrain(sd, projectDir, pending, log));
  let timer = null;
  let running = false;
  let stopped = false;
  ...
  async function tick() {
    ...
    try {
      await drainFn(specsDir, pending);
    } catch (e) {
