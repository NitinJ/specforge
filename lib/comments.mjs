...
/**
 * When the newest comment in these threads was written, in ms (0 for none).
 *
 * A spec's activity is not only its meta stamps — a thread that moved is
 * activity too — and this is the comment half of that number.
 * lib/collections.mjs orders the groups by it.
 */
export function newestCommentAt(threads) {
  let at = 0;
  for (const thread of threads || []) {
    for (const c of thread.comments || []) {
      const t = typeof c.createdAt === 'number' ? c.createdAt : Date.parse(c.createdAt) || 0;
      if (t > at) at = t;
    }
  }
  return at;
...
export function addComment(store, threadId, {
  body, author = 'human', kind: explicitKind, batchId, editedSpec, effectKey,
  ...
  if (effectKey) comment.effectKey = effectKey;
