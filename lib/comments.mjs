...
export function addComment(store, threadId, { body, author = 'human', batchId, editedSpec, now = new Date().toISOString() }) {
  ...
  // (and the action button) treat it as live again. A reply to an already-open or
  // agent-`replied` thread is left as-is (the conversation continues).
  if (author === 'human' && thread.state === 'resolved') thread.state = 'open';
  return comment;
}
