...
import { liveSessions, attach, detach } from './attach.mjs';
 ...
 * @returns {{status:number, body:object}}
 */
export function handleTypeDelete(slug) {
  ...
  // Row first, then the spec. If the process died between them the kind would
  // be gone with an orphan template spec left on the index, which is visible and
  // removable; the other order leaves a kind whose template does not exist,
  // which scaffolds nothing and looks like a bug in create.
  const templateId = templateIdFor(slug);
  const removedRow = removeCustomType(slug);
  // Detached first so no session keeps a reverse-index entry pointing at a
