...
import { SHELL_CLASSES } from './shell-classes.mjs';
import { VERSION } from './components-build.mjs';
import { optedInVersion } from './components-stamp.mjs';
...
/**
 * Classes that are not components but are not drift either: the shell a spec is
 * authored in. Derived from the templates by `components build` rather than
 * listed here, because the list that was here named eight and the deck shell
 * alone defines 43.
 */
const IGNORED = SHELL_CLASSES;
...
export function checkComponents(html) {
  ...
  const optedVersion = optedInVersion(html);
  if (optedVersion === null) return { applies: false, problems: [] };
  ...
  else if (Number(start[1]) !== VERSION || optedVersion !== VERSION) {
