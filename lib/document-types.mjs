...
export const DOCUMENT_TYPES = [
  ...
  // ---------------------------------------------------------------- 1 pager --
  {
    name: 'Design 1 pager',
    slug: 'design-1-pager',
    whenToUse: 'A design 1 pager: a direction to approve before anyone writes the full '
      + 'design. One page holding the problem, the approach, what it gains, what it '
      + 'costs, and the decisions needed before it is written up. Pick this when '
      + 'the work is large enough that a rejected design spec would waste a day. '
      + 'The full design-impl spec follows once the direction is approved.',
    sections: [
      {
  ...
  // -------------------------------------------------------------------- PRD --
  {
    name: 'PRD',
    slug: 'prd',
    whenToUse: 'A PRD: what we are building and what done means, from the product side. '
      + 'Users, requirements, the experience, and the numbers that say it worked. '
      + 'Pick this when the question is what the product should do; pick design or '
      + 'design-impl when the question is how it should be built.',
    sections: [
      {
  ...
  // -------------------------------------------------------- Marketing spec --
  {
    name: 'Marketing spec',
    slug: 'marketing-spec',
    whenToUse: 'A marketing spec for a piece of marketing work: what we are saying, to '
      + 'whom, where it runs, what has to be made, and how we know it worked. Pick '
      + 'this for a campaign, a launch message, a landing page or a content push. '
      + 'Not for the product itself, which is a PRD.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ---------------------------------------------------------------- UX spec --
  {
    name: 'UX spec',
    slug: 'ux-spec',
    whenToUse: 'A UX spec: the interaction design of one piece of product. The flows, '
      + 'every state a screen can be in, the words on it, and the keyboard and '
      + 'screen reader behaviour. Pick this before a frontend design spec, which is '
      + 'how it gets built.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ---------------------------------------------------------- Frontend spec --
  {
    ...
    whenToUse: 'A frontend design spec: how a piece of UI is built. Component structure, '
      + 'where state lives, how data is fetched, styling, performance budget and '
      + 'tests. Pick this after a UX spec has settled what the interface does.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ------------------------------------------------------------ Exploration --
  {
    ...
    whenToUse: 'An exploration that maps a subject nobody here knows yet: what exists, what the '
      + 'words mean, and which few things decide outcomes. The output is '
      + 'understanding, not a verdict, and you cannot yet state the question '
      + 'precisely. Pick research instead when there is one specific question with '
      + 'a right answer, and code-exploration-spec when the subject is this '
      + 'codebase.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ------------------------------------------------------- Code exploration --
  {
    ...
    whenToUse: 'A code exploration: find out how something in this codebase actually works '
      + 'and write it down so the next person does not repeat the reading. Entry '
      + 'points, real behaviour, dependents, traps, and where a change would go. '
      + 'Pick this over exploration-spec when the subject is code we own, and over '
      + 'research when the answer is in the repo rather than outside it.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ----------------------------------------------------- Design exploration --
  {
    ...
    whenToUse: 'A design exploration: generate several ways to solve a problem and compare '
      + 'them before committing to one. Three to five options, each sketched far '
      + 'enough to argue with, then a recommendation. Pick this when the solution '
      + 'space is genuinely open, and a design 1 pager when there is already one '
      + 'approach worth approving.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // -------------------------------------------------------- Security review --
  {
    ...
    whenToUse: 'A security review of something we built, for the ways it can be abused: '
      + 'what an attacker wants, where the trust boundaries are, what was found, '
      + 'and what gets fixed in what order. Pick this for a feature, a service or a '
      + 'dependency surface, before it is exposed to anyone.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ------------------------------------------------------------- Test plan --
  {
    name: 'Test plan',
    slug: 'test-plan',
    whenToUse: 'A test plan for one change: what has to be true before it ships and what '
      + 'proves it. Automated coverage per behaviour, what only a human can check, '
      + 'the data it needs, and what is deliberately untested. Pick this per '
      + 'feature. It is not the codebase-wide testing strategy.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // -------------------------------------------------------- Product phasing --
  {
    ...
    whenToUse: 'A product phasing spec: break a body of work into phases, each ending in a '
      + 'capability the team or a user gains. Says what forces the order, what ends '
      + 'each phase, and what is deferred to which one. Pick this before an '
      + 'implementation plan, which sequences PRs inside a phase.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ------------------------------------------------------------ Launch plan --
  {
    name: 'Launch plan',
    slug: 'launch-plan',
    whenToUse: 'A launch plan for getting something out: what must be true before we go, '
      + 'the ordered sequence, how to undo it, what to watch and for how long. Pick '
      + 'this for a deploy, a store submission, an enablement or a public '
      + 'announcement.',
    sections: [
      {
        id: 'tldr', toc: 'TL;DR', heading: 'TL;DR', panel: true,
  ...
  // ----------------------------------------------------------------- Review --
  // Written by a reviewer agent about somebody else's spec, and always filed as
  // that spec's child by `specforge review`. Four sections on purpose: a review
  // the author cannot read in five minutes does not get acted on.
  {
    name: 'Review',
    slug: 'review',
    whenToUse: 'A review of another spec, written by a reviewer agent and filed as a '
      + 'child of the spec it reviews: a verdict, findings ranked by severity, and what '
      + 'the author should change. Create it with the review command rather than by '
      + 'picking this type. Pick security-review instead when the subject is how '
      + 'something we built can be abused.',
    sections: [
      {
        id: 'tldr', toc: 'Verdict', heading: 'Verdict', panel: true,
        body: '      <p style="margin-bottom:0"><strong>{{ approve · approve with changes · rework }}</strong> {{ The one change that matters most, in a sentence. }}</p>',
        prompt: P(
          'One verdict from three: approve, approve with changes, or rework. Then the',
          'single change that matters most. Two sentences in total, written last. A',
          'reviewer who cannot pick a verdict has not finished reading.',
        ),
      },
      {
        id: 'scope', toc: '1 · What I reviewed', heading: '1 · What I reviewed',
        body: '    <p>{{ The spec, by id and link, and what it was checked against: the code, a sibling spec, the request. }}</p>\n'
          + '    <p class="sub">Not reviewed: {{ … }}</p>',
        prompt: P(
          'The spec by id and link, and what it was checked against: the code at a',
          'commit, a sibling spec, the original request, or nothing but itself. Then',
          'what was not reviewed. A review with no stated boundary reads as covering',
          'everything. Say so here if you are also the spec\'s author.',
        ),
      },
      {
        id: 'findings', toc: '2 · Findings', heading: '2 · Findings',
        body: '    <table>\n'
          + '      <thead><tr><th style="width:42px">#</th><th>Finding</th><th>Where</th><th style="width:90px">Severity</th><th>Change</th></tr></thead>\n'
          + '      <tbody><tr><td>F1</td><td>{{ what is wrong, quoting the spec where it turns on its words }}</td><td>{{ the section, linked }}</td><td><span class="tag bad">blocking</span></td><td>{{ what the author should do, in one line }}</td></tr></tbody>\n'
          + '    </table>',
        prompt: P(
          'One row per finding, most severe first. Severity is blocking (the spec is',
          'wrong, or cannot be built from), major (it will cause rework), or minor',
          '(wording, order, a missing link). Where names the section, linked as',
          '/spec/<parent id>#<section id>. Change says what the author should do, in',
          'one line. No praise rows: a finding is something to change. Cut minor',
          'findings that do not change what a reader understands.',
        ),
      },
      {
        id: 'questions', toc: '3 · Questions for the author', heading: '3 · Questions for the author',
        body: '    <ul><li>{{ A question only the author can answer, and which finding it would change. }}</li></ul>',
        prompt: P(
          'Only questions whose answer changes a finding or the verdict, each naming',
          'which. Anything you could have looked up is a finding or nothing. Write',
