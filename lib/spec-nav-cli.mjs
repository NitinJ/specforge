...
function resolveTarget(flags) {
  ...
  // relPath is intentionally the basename (no project context) — this CLI is
  // standalone and only builds the index in-memory, never persists it, so the id
  // need not be globally unique. (Differs from v1 paths.mjs, which keyed ids by
  // specsDir-relative path; do NOT feed a basename-keyed spec to writeIndex.)
