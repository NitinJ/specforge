 ...
 *
 * Keyed on thread, batch and action together, because every smaller key leaves a
 * hole that is reachable in ordinary use:
 *
 *   section + action   a draft on the same section from another conversation
 *                      closes this one
 *   thread             a thread accumulates across batches, and one comment can
 *                      name two actions, so the first draft answers for all
 *   thread + action    "@visualize" then "@visualize again, as a table" is two
 *                      requests with different qualifiers, and the first diagram
 *                      closes the second
 *
 * Thread plus batch plus action is the request itself. An aside missing any of
 * the three answers nothing, which is strict on purpose: the alternatives all
 * reopen one of the holes above, and `--force` is the way through for a draft
 * written by hand.
 */
function answeredBy(html, { threadId, batchId, actionId }) {
  for (const id of getAsideSectionIds(html)) {
    ...
    if (attrs.includes(`data-sf-thread="${threadId}"`)
      && attrs.includes(`data-sf-action="${actionId}"`)
      && (!batchId || attrs.includes(`data-sf-batch="${batchId}"`))) return true;
  }
  return false;
}
...
export function asideGaps(threads, html, ctx) {
  ...
  for (const t of threads || []) {
    for (const a of actionsForThread(t, ctx)) {
      ...
      if (answeredBy(html, { threadId: t.id, batchId: a.batchId, actionId: a.id })) continue;
