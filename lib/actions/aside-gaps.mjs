 ...
 *
 * Keyed on the thread rather than on section plus action. Those two are not
 * enough: ask `@visualize` on §object today and an aside written on §object last
 * week satisfies the check, so the gate passes with no new draft. The thread id
 * is what makes a request distinguishable from the one before it, which is why
 * `run` always carries `--thread`.
 */
function answeredBy(html, threadId) {
  for (const id of getAsideSectionIds(html)) {
    const m = html.match(new RegExp(`<section\\b[^>]*\\bid="${id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>`));
    if (m && m[0].includes(`data-sf-thread="${threadId}"`)) return true;
  }
  return false;
}
...
export function asideGaps(threads, html, ctx) {
  ...
  for (const t of threads || []) {
    for (const a of actionsForThread(t, ctx)) {
      ...
      if (answeredBy(html, t.id)) continue;
