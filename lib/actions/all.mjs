...
/**
 * The registry as the user has it: shipped actions with their text overrides
 * applied, then their custom ones.
 *
 * The merge lives inside this module's existing exports rather than beside them,
 * so every consumer inherits customization with no call-site change and there is
 * no second API a future consumer could import instead and silently skip it
 * (spec 094abd0b9d, D11). `SHIPPED_ACTIONS` stays exported for the settings page,
 * which has to show what a reset would restore.
 *
 * Hidden ids are NOT removed here. Hiding is a menu decision; resolution has to
 * keep answering, because an id inside a comment sent months ago must resolve
 * whatever the menu looks like today (D6). `menuActions` and `forScope` drop
 * them; `actionById` does not.
 */
export const SHIPPED_ACTIONS = ALL_ACTIONS;
...
function effective() {
  ...
  try {
    ...
    // A settings file must never stop the product working. Unreadable reads as
    // uncustomized, which is the state every store ships in.
    return { list: SHIPPED_ACTIONS, hidden: new Set(), tombstones: [] };
  }
  const cfg = (prompts && prompts.actions) || {};
  const overrides = cfg.overrides || {};
  const list = SHIPPED_ACTIONS.map((a) => {
    const o = overrides[a.id];
    if (!o) return a;
    return { ...a, ...(o.instruction ? { instruction: o.instruction } : {}),
      ...(o.importInstruction ? { importInstruction: o.importInstruction } : {}) };
  });
  for (const c of cfg.custom || []) {
    try {
      list.push(defineAction(c));
    } catch {
      // A definition this module refuses is dropped rather than thrown: the
      // store's sanitize already accepted it, so reaching here means the two
      // disagree, and a settings file cannot be allowed to break the menu.
    }
  }
  return { list, hidden: new Set(cfg.hidden || []), tombstones: cfg.tombstones || [] };
}

/** Every action in force: shipped plus custom, overrides applied. */
 ...
 *
 * Answers for hidden actions and for deleted custom ones, from their tombstone.
 * This is the promise the id-not-instruction rule rests on: a comment carries an
 * id, and that id has to mean something for as long as the comment exists.
 */
export function actionById(id) {
  if (typeof id !== 'string' || !id) return null;
  const { list, tombstones } = effective();
  const found = list.find((a) => a.id === id);
  if (found) return found;
  const dead = tombstones.find((a) => a.id === id);
  if (!dead) return null;
  try {
    return defineAction(dead);
  } catch {
    return null;
  }
}
...
export function forScope(scope) {
  const { list, hidden } = effective();
  return list.filter((a) => a.scope === scope && !hidden.has(a.id));
}
