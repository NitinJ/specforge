...
/**
 * What importing this action's aside does to the section it came from.
 *
 * `merge` adds the draft to what is there, beside the block it was asked about.
 * `replace` puts it in place of the section, which is right where the output
 * supersedes its input rather than adding to it: a diagram of a section is that
 * section afterwards, and keeping the prose beside it leaves the same thing said
 * twice.
 *
 * Only meaningful on an aside action. Declared per action rather than inferred,
 * because replacing a section is destructive and should be somebody's decision.
 */
 ...
 *            instruction:string, needsDetail:boolean}}
 */
export function defineAction(a) {
  ...
  const importMode = a.importMode === undefined ? 'merge' : a.importMode;
  if (!IMPORT_MODES.includes(importMode)) {
    throw new Error(`action ${a.id}: unknown importMode ${JSON.stringify(a.importMode)}, expected one of ${IMPORT_MODES.join(', ')}`);
  }
  // Only an aside is ever imported, so a mode on anything else is a statement
  // about a thing that never happens, and reads as though it does.
  if (importMode === 'replace' && a.kind !== 'aside') {
    throw new Error(`action ${a.id}: importMode is only meaningful on an aside action, and this is ${a.kind}`);
  }
  return {
    id: a.id,
    label: a.label.trim(),
    ...
    // Only read for an aside action. `merge` on everything else, which is the
