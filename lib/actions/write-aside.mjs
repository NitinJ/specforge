...
import { escapeRe, endOfElement, sectionRange } from './section-span.mjs';
 ...
 */
export function writeAside(html, { section, action, body, block, thread, batch } = {}) {
  ...
  // Which request this draft answers, as thread plus batch. Neither alone is
  // enough: without the thread, a draft on the same section from another
  // conversation closes this one; without the batch, "@visualize" followed by
  // "@visualize again, as a table" is answered by the first diagram.
  const threadAttr = /^th_[a-z0-9]+$/i.test(String(thread || '')) ? ` data-sf-thread="${thread}"` : '';
  const batchAttr = /^b_[a-z0-9]+$/i.test(String(batch || '')) ? ` data-sf-batch="${batch}"` : '';
  // No heading. The draft carries `data-sf-action`, and every surface that shows
  // one reads its label from there: the panel header, the strip on the draft and
  // the marker. A stored `<h3>Aside: Visualize</h3>` put the same words on screen
  // a second line below the first, and named an internal concept while doing it.
  const el = `\n  <section id="${id}" data-sf-aside="${section}"${blockAttr}${threadAttr}${batchAttr} data-sf-action="${a.id}">\n`
    + `    ${inner}\n  </section>`;
