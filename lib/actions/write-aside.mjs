 ...
 */
export function writeAside(html, { section, action, body, block, thread } = {}) {
  ...
  // The thread this draft answers. It is what lets batch-done tell a new request
  // from one already satisfied: without it, an aside written last week on the
  // same section with the same action passes today's gate.
  const threadAttr = /^th_[a-z0-9]+$/i.test(String(thread || '')) ? ` data-sf-thread="${thread}"` : '';
  const el = `\n  <section id="${id}" data-sf-aside="${section}"${blockAttr}${threadAttr} data-sf-action="${a.id}">\n`
    + `    <h3>Aside: ${a.label}</h3>\n    ${inner}\n  </section>`;
