...
function nextFor(action, { section, run, detail }) {
  ...
  if (action.scope === 'aside') {
    // @import and @dismiss arrive from the buttons on an aside, so the comment
 ...
 */
export function actionsForThread(thread, { specId, cli, batchIds, html } = {}) {
  ...
  for (const c of liveComments(thread, batchIds)) {
    for (const a of parseActions(c.body)) {
      ...
      const target = a.id === 'import' && html ? importTarget(html, section) : null;
      out.push({
        id: a.id,
        label: a.label,
        kind: a.kind,
