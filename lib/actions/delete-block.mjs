...
export function decodeEntities(s) {
  // Case is significant for a NAME and not for a hex reference, so the branches
  // spell their own case rather than sharing an `i` flag. Sharing one is what
  // made `&Alpha;` decode as `&alpha;`; dropping it without widening the hex
  // branch is what made `&#X2014;` and `&#x201C;` stop decoding at all.
