...
export function codemod(html) {
  ...
  const out = html.replace(TAG_RE, (tag, name, attrs, selfClose) => {
    ...
    const attrsNext = next.length
      ? attrs.replace(CLASS_RE, `class="${next.join(' ')}"`)
      : attrs.replace(/\s*\bclass\s*=\s*"[^"]*"/i, '');
    return `<${name}${attrsNext}${selfClose}>`;
  });
...
const CALLOUT_RE = /<div\b([^>]*\bclass\s*=\s*"([^"]*)"[^>]*)>([\s\S]*?)<\/div>/gi;
const LEGACY_SOURCES = new Set(['warn', 'good', 'bad']);

/**
 * Callouts still carrying no type: what the agent pass reads.
 *
 * `index` is the occurrence number among callouts in document order, which is
 * how an assignment names a block. Stable for the length of one migration, which
 * is all it has to be — the work list and the apply run off the same pass.
 */
...
function applyType(html, index, type) {
  let seen = 0;
  return html.replace(CALLOUT_RE, (full, attrs, cls, body) => {
    ...
    return `<div${attrs.replace(CLASS_RE, `class="${next.join(' ')}"`)}>${body}</div>`;
  });
}
 ...
 *
 * @param {string} id
 * @param {{dry?:boolean, assign?:Record<string|number,string>, now?:number}} [opts]
 */
export function migrateSpec(id, opts = {}) {
  ...
  const assignments = [];
  for (const block of ambiguousBlocks(html)) {
    const chosen = assign[block.index];
