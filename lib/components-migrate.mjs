...
/**
 * A block's identity for an assignment: the first 10 hex of its text hash.
 *
 ...
 * of the text either finds the block the agent read or finds nothing, and
 * finding nothing is reported.
 *
 * Two blocks with identical text share a key and take the same type, which is
 * the right answer: identical text classifies identically.
 */
...
export function ambiguousBlocks(html) {
  ...
  while ((m = CALLOUT_RE.exec(html))) {
    ...
    out.push({ index: i, key: blockKey(text), source, text });
  }
  return out;
}
