...
// the spec to the current session, whose key the running harness supplies
// (lib/harness/). Skills drive the authoring (HTML); this CLI owns the store,
// the lock and the daemon plumbing.
//
...
import {
  attach, detach, specsForSession, specsConnectedTo, heartbeat, setWatcher, clearWatcher,
  watcherAlive, isStale, HEARTBEAT_MS,
} from './attach.mjs';
import { connect, connectionList, activeHarnessOf, canWrite, writeRefusal } from './connections.mjs';
import { sameSession } from './session-key.mjs';
...
import { currentHarness, currentSessionKey } from './harness/index.mjs';
import { parseKey } from './session-key.mjs';
...
/**
 * The session this CLI invocation belongs to, as a harness-qualified key.
 *
 * `--session` still wins, and is passed through as given: a caller naming a
 * session explicitly means that one, and normalising happens where the key is
 * compared rather than here.
 */
function sessionId(deps) {
  return deps.session !== undefined ? deps.session : currentSessionKey();
}
...
export async function cmdCreate({ title, origin = null, type = DEFAULT_TYPE, project } = {}, deps = {}) {
  ...
  if (proj) {
    const { mutateMeta } = await import('./meta.mjs');
    mutateMeta(id, (meta) => ({ ...meta, project: proj }));
  }
  const session = sessionId(deps);
  if (session) attach(id, session);
  // The authoring preamble rides beside the section prompts: both are guidance
  // the agent needs before it writes, and both are empty on an uncustomized
  // store, which is what every store ships as.
  const { readPrompts } = await import('./store-prompts.mjs');
  const language = readPrompts().language || '';
  return {
    id,
...
export async function cmdImport({ file, title, type = DEFAULT_TYPE } = {}, deps = {}) {
  ...
  return {
    ...
    language: readPrompts().language || '',
    ...armWatcherNext(session),
  };
}

/**
 * Connect this session to an existing spec and return its url (open from index).
 *
 * Becomes the active harness only when no harness is (§8 Q7). `open` is an agent
 * action, and no agent action may take work from another: the human switches, in
 * the spec header. The one exception is the empty case, where becoming active
 * takes nothing from anyone and is what makes a spec its creator just made
 * immediately workable.
 */
export async function cmdOpen({ id } = {}, deps = {}) {
  if (!id) throw new Error('open: <id> required');
  const meta = readMeta(id);
  if (!meta) throw new Error(`open: unknown spec ${id}`);
  const ensure = deps.ensureDaemon || realEnsureDaemon;
  const session = sessionId(deps);
  let active = activeHarnessOf(meta);
  if (session) {
    const mine = sameSession(meta.attachedSession, session);
    if (!meta.attachedSession || mine || isStale(meta)) {
      attach(id, session); // throws if locked by another live session
      active = activeHarnessOf(readMeta(id));
    } else {
      // Another harness is working it. Join as a connection and say so, rather
      // than refusing: connecting is what lets this session read and reply.
      connect(id, session);
    }
  }
  const { url } = await ensure();
  const now = readMeta(id);
  return {
    id,
    url: specUrl(url, id),
    active,
    connections: connectionList(now).map((c) => c.harness),
    ...(session && !sameSession(now.attachedSession, session)
      ? { note: `connected, but the ${active} harness is working this spec. Switch it in the spec header to take over.` }
      : {}),
    ...armWatcherNext(session),
  };
}
...
/**
 * Tell a session with no watcher to arm one.
 *
 * Connecting a spec and being able to hear from it are two different things, and
 * every command here does only the first. The watcher cannot be armed on the
 * agent's behalf: its exit IS the wake-up, so it has to be a background task the
 * agent is itself waiting on, and a process this CLI spawned has no channel back.
 *
 * What the CLI can do is say so at the one moment the agent is certainly
 * reading, which is the answer to the command it just ran. The create skill said
 * it; nothing else did, so a spec attached any other way sat connected and deaf
 * until a human noticed the browser saying Disconnected.
 *
 * Silent when a watcher is already beating: repeating it arms a second one, and
 * two watchers on one session race for the same batch.
 */
function armWatcherNext(session) {
  if (!session || watcherAlive(session)) return {};
  return {
    next: 'Arm the review watcher in the background so submitted comments reach you '
      + `while you are idle: ${WATCH_CMD}`,
  };
}
 ...
 */
/**
 * Report the five things a human needs when the review loop is not working, and
 * nothing else (spec e9ddcddef6, Q5, E9).
 *
 * The scope is fixed deliberately: no flags, no output modes, no formatting
 * options. What it exists for is the session-key migration, which fails silently
 * when it fails at all, and this is how a person sees that it landed.
 *
 * @returns {{harness, session, specs, watcher, onPath}}
 */
export function cmdDoctor(deps = {}) {
  const env = deps.env || process.env;
  const harness = currentHarness(env);
  const session = deps.session !== undefined ? deps.session : currentSessionKey({ env, harness });
  // Connected, not merely active: a session working nothing but listening to a
  // spec another harness is writing still owns a connection, and a person asking
  // "why are my comments not reaching anyone" needs to see it.
  const specs = session ? specsConnectedTo(session) : [];
  return {
    harness: { id: harness.id, agent: harness.agentName, detected: !env.SPECFORGE_HARNESS },
    session: session
      ? { key: session, raw: parseKey(session).raw }
      : { key: null, raw: null, why: `the ${harness.id} harness could not name this session` },
    specs: specs.map((id) => {
      const meta = readMeta(id) || {};
      return {
        id,
        title: meta.title || id,
        active: activeHarnessOf(meta),
        mine: sameSession(meta.attachedSession, session),
        connected: connectionList(meta).map((c) => `${c.harness}${c.alive ? '' : ' (reconnect)'}`),
      };
    }),
    watcher: { alive: session ? watcherAlive(session) : false, command: WATCH_CMD },
    // A skill that says `specforge <verb>` fails obscurely when the bin is not
    // linked, so the answer belongs beside the rest rather than in a stack trace.
    onPath: binOnPath(env),
  };
}

/**
 * Where SpecForge is installed.
 *
 * Skills used to name this with a plugin-root variable only one CLI expands.
 * Computed from this file's own location, so it is right on every harness and
 * needs nothing from the environment.
 */
export function cmdRoot() {
  return resolve(dirname(fileURLToPath(import.meta.url)), '..');
}

/**
 * Refuse a write from a harness that is not the active one (E7, I6).
 *
 * Called by every path that writes `spec.html`, and by none that writes a
 * comment: a connected harness that is not active can still read and answer,
 * which is what makes connecting a second one worth doing (I10).
 */
function requireWritable(id, meta, verb) {
  const me = currentSessionKey();
  // No session at all is a script or a test, not a competing agent.
  if (!me) return;
  if (canWrite(meta, me)) return;
  throw new Error(`${verb}: ${writeRefusal(id, meta)}`);
}

/**
 * The reference documents a skill tells an agent to read, by short name.
 *
 * Skills used to name these through a plugin-root variable only one CLI expands.
 * A short name beats making the skill build a path two ways:
 * `$(specforge root)/references/x.md` works on every harness but is a command
 * substitution, which Claude Code's permission matcher cannot resolve, so every
 * read would stop for a manual approval.
 */
const DOCS = {
  'spec-language': 'references/spec-language.md',
  'spec-components': 'references/spec-components.md',
  'house-rules': 'templates/house-rules.md',
};

/**
 * Print a reference document, or list what there is.
 *
 * Prints the contents rather than the path: one step on every harness, and no
 * assumption about what the caller can read.
 */
export function cmdDoc({ name } = {}) {
  if (!name) {
    return `documents: ${Object.keys(DOCS).join(', ')}\nusage: specforge doc <name>`;
  }
  const rel = DOCS[name];
  if (!rel) {
    throw new Error(`doc: unknown document "${name}" — try one of: ${Object.keys(DOCS).join(', ')}`);
  }
  try {
    return readFileSync(join(cmdRoot(), rel), 'utf8');
  } catch (e) {
    throw new Error(`doc: cannot read ${rel}: ${e.message}`);
  }
}

/**
 * Every comment thread across the store, for the skill that tunes templates
 * from what reviewers keep saying.
 *
 * A verb rather than a path into the install, for the same reason `doc` is one.
 */
export async function cmdMineComments() {
  const { mineComments } = await import('./mine-comments.mjs');
  return mineComments();
}

/**
 * Lint a spec file, so a skill can say `specforge lint <path>` rather than
 * naming a path inside the install.
 *
 * Prints the same report `lib/lint-spec.mjs` prints and exits the same way,
 * because the skills' verify steps read that output.
 */
export function cmdLint({ file } = {}) {
  if (!file) throw new Error('lint: <spec.html> required');
  let html;
  try {
    html = readFileSync(file, 'utf8');
  } catch (e) {
    throw new Error(`lint: cannot read ${file}: ${e.message}`);
  }
  const { ok, checks } = lintSpec(html);
  for (const c of checks) {
    process.stdout.write(`${c.ok ? '✓' : c.advisory ? '⚠' : '✗'} ${c.name} — ${c.detail}\n`);
  }
  process.stdout.write(ok ? '\nlint: PASS\n' : '\nlint: FAIL\n');
  if (!ok) process.exitCode = 1;
  return { ok, checks: checks.map((c) => ({ name: c.name, ok: c.ok, advisory: !!c.advisory })) };
}

/**
 * Is a `specforge` executable resolvable from PATH?
 *
 * Executable, not merely present. Every skill says `specforge <verb>`, so a file
 * this process cannot run is a skill that fails with "permission denied" while
 * doctor reports the CLI as installed, which is the one answer worse than "not
 * found". `accessSync(X_OK)` is the same question the shell asks.
 *
 * On Windows the execute bit means nothing, so a readable `.cmd` or `.exe`
 * counts: `X_OK` there is documented as equivalent to `F_OK`.
 */
function binOnPath(env = process.env) {
  const dirs = String(env.PATH || '').split(delimiter).filter(Boolean);
  for (const dir of dirs) {
    for (const name of ['specforge', 'specforge.cmd', 'specforge.exe']) {
      const full = join(dir, name);
      try {
        if (!statSync(full).isFile()) continue;
        accessSync(full, constants.X_OK);
        return full;
      } catch { /* not here, or here and not runnable */ }
    }
  }
  return null;
}
...
export async function cmdWaitBatch({ timeout = null, interval = HEARTBEAT_MS / 1000 } = {}, deps = {}) {
  ...
  if (!session) {
    throw new Error(
      `wait-batch: no session id — the ${currentHarness().id} harness could not name this session. `
      + 'Pass --session <id>.',
    );
  }
  ...
  for (;;) {
    ...
    if (pending.length) {
      return {
        ready: true,
        pending,
        // The skill is named the way THIS CLI names it. A hard-coded plugin id
        // reads as an address under one CLI and as nothing at all under another,
        // which is the whole reason workRef exists.
        next: `Run the ${currentHarness().workRef('review-spec')} skill for each pending spec, `
          + `then re-arm this watcher in the background: ${WATCH_CMD}`,
      };
    }
...
export async function cmdComments({ id } = {}) {
  ...
  const threads = loadComments(id).threads.map((t) => {
  ...
  const { readPrompts } = await import('./store-prompts.mjs');
  const language = readPrompts().language || '';
  // Whether this session may amend the spec, and why not when it may not.
  //
  // An agent amends `spec.html` with its own editor, which nothing here can
  // gate, so the refusal has to arrive before the edit rather than at it. Two
  // harnesses can be connected and only one is active (I10): the other still
  // reads the threads and still replies, which is what carries the answer back
  // when the reader asked the wrong agent.
  const meta = readMeta(id);
  const me = currentSessionKey();
  const writable = !me || canWrite(meta, me);
  return {
    specId: id,
    htmlPath: specHtmlPath(id),
    language,
    ...
    pending,
    writable,
    ...(writable ? {} : { refusal: writeRefusal(id, meta) }),
  };
}
...
export async function cmdReply({ id, tid, body } = {}) {
  ...
  // Only this path writes agent comments. `kind` is what marks them; the name is
  // display only, so a person called claude cannot reply as the agent. The name
  // comes from the running harness, so a thread carrying replies from two CLIs
  // says which wrote which.
  const author = currentHarness().agentName;
  const comment = mutateComments(id, (store) => addComment(store, tid, { body, author, kind: 'agent' }));
...
export async function cmdImportMd({ file, title, type, date, owner } = {}, deps = {}) {
  ...
  // the user's.
  const { readPrompts } = await import('./store-prompts.mjs');
  return { ...result, url: specUrl(url, result.id), language: readPrompts().language || '' };
}
...
export async function cmdAside({ id, section, action, file, body, block, thread, batch } = {}) {
  ...
  // An aside is a section spliced into spec.html, so it needs the active harness
  // (§8 Q6). Replying to a comment does not, which is what a connected but
  // inactive harness can still do.
  requireWritable(id, meta, 'aside');
...
/**
 * Commands whose output a shell reads, not an agent.
 *
 * `$(specforge root)` has to yield a path; wrapping it in JSON would embed the
 * quotes in whatever the skill built with it. `specforge lint` has already
 * written its report to stdout by the time it returns, and printing the return
 * value too would put a JSON dump under it.
 */
const RAW_COMMANDS = new Set(['root', 'lint', 'doc']);
...
export const COMMANDS = Object.assign(Object.create(null), {
  ...
  doctor: () => cmdDoctor(),
  root: () => cmdRoot(),
  lint: (p) => cmdLint({ file: p[0] }),
  doc: (p) => cmdDoc({ name: p[0] }),
  'mine-comments': () => cmdMineComments(),
});
...
async function main() {
  ...
  try {
    ...
    if (RAW_COMMANDS.has(cmd)) {
      // These two are read by a shell rather than by an agent. `$(specforge
      // root)` must produce a path, not a JSON string complete with quotes, and
      // `specforge lint` has already printed its report.
      if (typeof result === 'string') process.stdout.write(`${result}\n`);
    } else if (REPORTING_COMMANDS.has(cmd) && flags.json !== true) {
