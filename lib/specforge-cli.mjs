 ...
 */
export async function cmdVerify({ id } = {}) {
  ...
  return { id, ...verifySpec(readSpecHtml(id), type) };
}
...
async function main() {
  ...
  try {
    ...
    // Three codes, because "a rule failed" and "a rule is yours to judge" call
    // for different things and one non-zero code cannot tell them apart.
    // 0 = nothing failed and nothing is outstanding
    // 1 = a blocking rule failed
    // 2 = nothing mechanical is broken; blocking judgements are outstanding
