...
export async function cmdReparent({ id, to, detach: doDetach = false } = {}, deps = {}) {
  ...
  if (to) {
    ...
    const { wouldCycle, descendantsOf } = await import('./spec-tree.mjs');
    if (wouldCycle(id, to)) {
      // Naming the chain, because "that would make a cycle" leaves the caller to
      // work out which of its own specs is in the way.
      throw new Error(`reparent: that would make a cycle: ${descendantsOf(id).join(' -> ')}`);
    }
  }

  const ensure = deps.ensureDaemon || realEnsureDaemon;
  const { url } = await ensure();
  const { writeMeta } = await import('./meta.mjs');
  const meta = readMeta(id);
  ...
  for (;;) {
    ...
    if (pending.length) {
      return {
        ready: true,
        pending,
        next: `Run the ${skillRef('review-spec', deps.env)} skill for each pending spec, then re-arm this watcher in the background: ${WATCH_CMD}`,
      };
    }
