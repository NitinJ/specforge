...
export async function cmdCreate({ title, origin = null, type = DEFAULT_TYPE, project } = {}, deps = {}) {
  ...
  return { id, htmlPath: specHtmlPath(id), url: specUrl(url, id), status: 'draft', type, project: proj, prompts };
}
...
export async function cmdComments({ id } = {}) {
  ...
  const threads = loadComments(id).threads.map((t) => {
    const actions = actionsForThread(t, { specId: id, cli });
 ...
 * imported or dismissed by the reader before the batch was closed.
 */
export async function cmdBatchDone({ id, batchId, force } = {}) {
  ...
  if (!force) {
    ...
    if (batch) {
      ...
      const gaps = asideGaps(threads, readSpecHtml(id), { specId: id, cli: fileURLToPath(import.meta.url) });
      if (gaps.length) {
        throw new Error(
          ...
          + '\n\nThen run batch-done again. If the reader already imported or dismissed it, pass --force.'
        );
      }
    }
  }
 ...
 */
export async function cmdAside({ id, section, action, file, body, block } = {}) {
  ...
  const out = writeAside(readSpecHtml(id), { section, action, body: content, block });
  writeSpecHtml(id, out.html);
  return { id, asideId: out.id, section, action, block: block || null };
}

export async function cmdActions({ id, scope } = {}) {
  const { ALL_ACTIONS, actionById, forScope } = await import('./actions/all.mjs');
...
export const COMMANDS = Object.assign(Object.create(null), {
  ...
  aside: (p, f) => cmdAside({
    id: p[0], section: f.section, action: f.action, file: f.file, body: f.body, block: f.block,
  }),
