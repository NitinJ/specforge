 ...
 * for a caller to parse, and whose exit code carries the verdict so a harness
...
async function main() {
  ...
  try {
    ...
    // Exit 0 only when the report says so, so a harness can gate on the code.
    if (REPORTING_COMMANDS.has(cmd) && result.ok === false) process.exit(1);
  } catch (err) {
