...
import {
  attach, detach, specsForSession, heartbeat, setWatcher, clearWatcher, watcherAlive, HEARTBEAT_MS,
} from './attach.mjs';
...
export function cmdDoctor(deps = {}) {
  ...
  const specs = session ? specsForSession(session) : [];
  return {
    ...
    specs: specs.map((id) => {
      const meta = readMeta(id) || {};
      return { id, title: meta.title || id, attached: meta.attachedSession || null };
    }),
