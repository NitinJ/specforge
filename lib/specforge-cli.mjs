...
async function creditReviewer(id, { model, harness } = {}, deps = {}) {
  try {
    const { agentIdentity, recordCredit, sameAgent } = await import('./credits.mjs');
    const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
    const meta = readMeta(id) || {};
    const author = meta.author || null;
    if (author && (sameAgent(author, agent) || (author.harness === agent.harness && !agent.model))) return null;
...
export async function cmdReview({ id, model, harness, title } = {}, deps = {}) {
  ...
  const { agentIdentity, recordCredit, creditLabel } = await import('./credits.mjs');
  const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
