...
//   specforge export-md <id> [--out path]         render the spec as GFM markdown
...
import { exportMd } from './store-md.mjs';
 ...
 * has to run.
 */
export async function cmdExportMd({ id, out, exportedAt } = {}) {
  if (!id) throw new Error('export-md: <id> required');
  return exportMd(id, { out, exportedAt });
}
...
const COMMANDS = {
  ...
  'export-md': (p, f) => cmdExportMd({ id: p[0], out: f.out, exportedAt: f.date }),
