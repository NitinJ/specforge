...
//   specforge review-wait [--timeout s] [--interval s] check once in Codex; otherwise wait until
...
import {
  attach, detach, specsForSession, heartbeat, claimWorker, releaseWorker, setSessionHarness,
  HEARTBEAT_MS,
} from './attach.mjs';
...
/**
 * Refuse an unknown --harness up front. The author credit is written after the
 * spec, and swallows its errors, so a bad flag caught there would leave a spec
 * behind with its credit silently missing.
 */
async function validateHarness(cmd, harness) {
  const { checkHarnessFlag } = await import('./credits.mjs');
  try {
    checkHarnessFlag(harness);
  } catch (e) {
    throw new Error(`${cmd}: ${e.message}`);
  }
}
...
/** Attach through one context path so browser state knows the owning harness. */
function attachForContext(id, deps) {
  const session = sessionId(deps);
  if (!session) return '';
  attach(id, session);
  setSessionHarness(session, resolveHarness(deps.env || process.env, deps.harness));
  return session;
}
...
export async function cmdCreate({
  title, origin = null, type = DEFAULT_TYPE, project, parent = null, model, harness,
} = {}, deps = {}) {
  ...
  await validateHarness('create', harness);
  let parentMeta = null;
  if (parent) {
  ...
  attachForContext(id, deps);
  const author = await creditAuthor(id, { model, harness }, deps);
  ...
  return {
    ...
    prompts,
    skeleton,
    // Who the spec is credited to, so the agent can see what it recorded.
    author,
  };
}
...
export async function cmdReparent({ id, to, detach: doDetach = false } = {}, deps = {}) {
  ...
  const { readMeta, mutateMeta } = await import('./meta.mjs');
  const { wouldCycle, ancestryOf } = await import('./spec-tree.mjs');

  if (!readMeta(id)) throw new Error(`reparent: spec not found: ${id}`);
  if (to) {
  ...
  // Through the lock, and re-read inside it: the daemon serves the same store,
  // and a watcher beat rewriting the file from a snapshot taken before this
  // would undo the move with nothing to restore it.
  // spec-tree-ok: sets this spec's own field, validated above
  mutateMeta(id, (meta) => ({ ...meta, parent: to || null }));
...
export async function cmdImport({ file, title, type = DEFAULT_TYPE, model, harness } = {}, deps = {}) {
  if (!file) throw new Error('import: <file> required');
  validateType('import', type);
  await validateHarness('import', harness);
  const ensure = deps.ensureDaemon || realEnsureDaemon;
  ...
  attachForContext(id, deps);
  // The agent converting the file is its author here: it re-authors or at least
  // takes ownership of what lands, and the source file names no agent.
  const author = await creditAuthor(id, { model, harness }, deps);
  ...
  return {
    ...
    language: languageContract(), author,
  };
}
...
export async function cmdOpen({ id } = {}, deps = {}) {
  ...
  const ensure = deps.ensureDaemon || realEnsureDaemon;
  // Open exists to attach. With no session there is nothing to attach to, and
  // reporting success would leave the spec owned by nobody.
  if (!sessionId(deps)) {
    throw new Error('open: no session id (SPECFORGE_SESSION_ID, CODEX_THREAD_ID, CODEX_SESSION_ID, and CLAUDE_CODE_SESSION_ID unset) — run it from the host\'s shell tool, or pass --session <id>');
  }
  attachForContext(id, deps); // throws if locked by another live session
  const { url } = await ensure();
...
/**
 * Check once in Codex; other hosts block until an attached spec has pending
 * work, then return its kind, items, and continuation guidance.
 *
...
export async function cmdWaitBatch({ timeout = null, interval = HEARTBEAT_MS / 1000 } = {}, deps = {}) {
  ...
  // Codex hooks deliver queued work on the next turn. A shell wait cannot wake
  // an idle thread, and sandbox-local PIDs cannot identify its worker globally.
  if (harness === CODEX_HARNESS) {
    setSessionHarness(session, harness);
    const delivery = pendingWorkForSession(session, { ...(deps.env || process.env), SPECFORGE_HARNESS: harness });
    return delivery ? deliveryResult(delivery) : { ready: false, pending: [], reason: 'next-turn' };
  }
...
/** Shared entry: a one-shot Codex check or the host's background delivery loop. */
...
/**
 * Credit the agent running this command as a spec's author.
 *
 * Never fails the command it rides on: a spec that was written is not unwritten
 * because its credit could not be recorded.
 *
 * @returns {Promise<{harness:string, model:string|null}|null>}
 */
async function creditAuthor(id, { model, harness } = {}, deps = {}) {
  try {
    const { agentIdentity, recordCredit } = await import('./credits.mjs');
    const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
    return recordCredit(id, 'author', agent).author || null;
  } catch {
    return null;
  }
}
...
/**
 * Credit the agent running this command as a reviewer, unless it is the author.
 *
 * The author answering comments on its own spec is still authoring. A reply
 * from the same harness that names no model is read as the author too, since
 * that is the usual case and a wrong reviewer credit is worse than a missing one.
 */
async function creditReviewer(id, { model, harness } = {}, deps = {}) {
  try {
    const { agentIdentity, recordCredit, isAuthor } = await import('./credits.mjs');
    const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
    const meta = readMeta(id) || {};
    if (isAuthor(meta.author, agent)) return null;
    return recordCredit(id, 'reviewer', agent).reviewers || null;
  } catch {
    return null;
  }
}
...
/**
 * Record an agent as a spec's author or reviewer, explicitly.
 *
 * The method for work the automatic credits cannot see: an agent that reviewed
 * a spec without replying to a thread, or a spec written before credits existed.
 * Harness is detected unless given; model is the agent's own id.
 */
export async function cmdCredit({ id, role, model, harness } = {}, deps = {}) {
  if (!id) throw new Error('credit: <id> required');
  const { CREDIT_ROLES, agentIdentity, recordCredit } = await import('./credits.mjs');
  if (!CREDIT_ROLES.includes(role)) {
    throw new Error(`credit: --role must be one of: ${CREDIT_ROLES.join(', ')}`);
  }
  if (!readMeta(id)) throw new Error(`credit: unknown spec ${id}`);
  const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
  const meta = recordCredit(id, role, agent);
  return { ok: true, id, role, agent, author: meta.author || null, reviewers: meta.reviewers || [] };
}

/**
 * Start a review of a spec: a child spec of type `review`, written by the agent
 * running this, credited to it, with that agent also credited as a reviewer of
 * the spec it reviews.
 *
 * A review is its own spec rather than a comment thread, so it has a URL, a
 * status and a review cycle of its own, and the parent's Child specs drawer
 * lists every review it has had. A second review is a second child: a re-review
 * after the author's changes never overwrites the first, so the history stays.
 *
 * Returns what `create` returns (id, htmlPath, url, skeleton, prompts,
 * language, author), plus which spec it reviews.
 */
export async function cmdReview({ id, model, harness, title } = {}, deps = {}) {
  if (!id) throw new Error('review: <specId> required');
  const reviewed = readMeta(id);
  if (!reviewed) throw new Error(`review: unknown spec ${id}`);
  const { agentIdentity, recordCredit, creditLabel, isAuthor } = await import('./credits.mjs');
  const agent = agentIdentity({ model, harness: harness || deps.harness }, deps.env || process.env);
  const created = await cmdCreate({
    title: title || `Review: ${reviewed.title || id} · ${creditLabel(agent)}`,
    type: 'review',
    parent: id,
    model: agent.model || undefined,
    harness: agent.harness,
  }, deps);
  let reviewers = Array.isArray(reviewed.reviewers) ? reviewed.reviewers : [];
  try {
    // An author reviewing its own spec is still authoring, as a reply is.
    if (!isAuthor(reviewed.author, agent)) {
      reviewers = recordCredit(id, 'reviewer', agent).reviewers || reviewers;
    }
  } catch {
    // The review exists either way; a missing credit is not worth failing it.
  }
  return { ...created, reviewOf: { id, title: reviewed.title || null }, reviewers };
}

/** Post an agent reply to a thread (the review flow's append-only reply). */
export async function cmdReply({ id, tid, body, effect, model } = {}, deps = {}) {
  ...
  if (!duplicate) await creditReviewer(id, { model }, deps);
 ...
 */
export async function cmdImportMd({ file, title, type, date, owner, model, harness } = {}, deps = {}) {
  if (!file) throw new Error('import-md: <file> required');
  if (type) validateType('import-md', type);
  await validateHarness('import-md', harness);
  const ensure = deps.ensureDaemon || realEnsureDaemon;
  const { url } = await ensure();
  const result = importMd(file, { title, type, date, owner });
  attachForContext(result.id, deps);
  const author = await creditAuthor(result.id, { model, harness }, deps);
  ...
  // the user's.
  const { languageContract } = await import('./language-contract.mjs');
  return { ...result, url: specUrl(url, result.id), language: languageContract(), author };
}
...
export const COMMANDS = Object.assign(Object.create(null), {
  ...
  create: (p, f) => cmdCreate({
    ...
    model: f.model, harness: f.harness,
  ...
  import: (p, f) => cmdImport({ file: p[0], title: f.title, type: f.type, model: f.model, harness: f.harness }),
  open: (p, f) => cmdOpen({ id: p[0] }, {
    ...(f.session ? { session: f.session } : {}),
    ...(f.harness ? { harness: f.harness } : {}),
  }),
  start: () => cmdStart(),
  listall: () => cmdListall(),
  list: () => cmdList(),
  ...
  reply: (p, f) => cmdReply({ id: p[0], tid: p[1], body: f.body, effect: f.effect, model: f.model }),
  credit: (p, f) => cmdCredit({ id: p[0], role: f.role, model: f.model, harness: f.harness }),
  review: (p, f) => cmdReview({ id: p[0], model: f.model, harness: f.harness, title: f.title }),
  ...
  'import-md': (p, f) => cmdImportMd({
    file: p[0], title: f.title, type: f.type, date: f.date, owner: f.owner, model: f.model, harness: f.harness,
  }),
