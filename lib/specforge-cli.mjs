...
export const BOOLEAN_FLAGS = new Set(['clear', 'rotate', 'write', 'force', 'install-service', 'zip']);
...
/**
 * `components <sub>`. One subcommand in this stage; sync and migrate arrive with
 * the stages that build them, so the dispatch names what it does not have yet
 * rather than reporting an unknown command.
 */
async function cmdComponents({ sub }) {
  ...
  const subs = Object.assign(Object.create(null), { build: cmdComponentsBuild });
  const fn = subs[sub];
...
const COMMANDS = Object.assign(Object.create(null), {
  ...
  components: (p) => cmdComponents({ sub: p[0] }),
