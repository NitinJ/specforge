 ...
 *
 * Skills used to name this with `${CLAUDE_PLUGIN_ROOT}`, which only one CLI
 * expands. Computed from this file's own location, so it is right on every
 * harness and needs nothing from the environment.
 */
 ...
 *
 * Skills used to name these with `${CLAUDE_PLUGIN_ROOT}/references/...`, which
 * only one CLI expands. A short name beats making the skill build a path two
 * ways: `$(specforge root)/references/x.md` works on every harness but is a
 * command substitution, which Claude Code's permission matcher cannot resolve,
 * so every read would stop for a manual approval.
 */
const DOCS = {
