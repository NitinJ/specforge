 ...
 */
export function formatRowsCompact(rows) {
  return rows.map((r) => [r.id, r.status, r.type, r.attached, r.title].join('  ')).join('\n');
}
...
async function main() {
  ...
  try {
    ...
    if ((cmd === 'list' || cmd === 'listall') && flags.json !== true) {
      process.stdout.write(formatRowsCompact(result.rows || []) + '\n');
