...
export async function cmdComments({ id } = {}) {
  ...
  const threads = loadComments(id).threads.map((t) => {
    const actions = actionsForThread(t, { specId: id, cli, batchIds });
 ...
 */
export async function cmdAside({ id, section, action, file, body, block, thread } = {}) {
  ...
  const out = writeAside(readSpecHtml(id), { section, action, body: content, block, thread });
  writeSpecHtml(id, out.html);
  return { id, asideId: out.id, section, action, block: block || null, thread: thread || null };
}
