...
export async function cmdContribute({ id, url, owner } = {}, deps = {}) {
  ...
  const { rememberContribution } = await import('./store-contributed.mjs');
  const { previous } = rememberContribution({
    origin: parsed.origin, token: parsed.token, specId: id, specToken: mine.share.token,
  });
  // A rotate since the last contribute left an entry over there under the old
  // token. Retire it, so the project lists this spec once rather than twice.
  if (previous) {
    await fetchImpl(`${parsed.origin}/p/${parsed.token}/contribute/${previous}`, { method: 'DELETE' })
      .catch(() => {});
  }
  return {
    ok: true, id, project: parsed.origin, token: mine.share.token, url: mine.url, replaced: previous,
  };
}
...
export async function cmdWithdraw({ id, url } = {}, deps = {}) {
  ...
  const { contributedToken, forgetContribution } = await import('./store-contributed.mjs');
  ...
  const token = contributedToken({ origin: parsed.origin, token: parsed.token, specId: id })
    || readShareToken(id);
  ...
  forgetContribution({ origin: parsed.origin, token: parsed.token, specId: id });
  return { ok: true, id, removed: !!body.removed };
}
