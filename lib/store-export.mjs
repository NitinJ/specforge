...
// surface it and route Claude to the export skill, and the skill reports
...
export function exportReason(metas, env = process.env) {
  const lines = metas.map((m) => `  - spec ${m.id} ("${m.title}")`);
  return [
    `SpecForge: ${metas.length} spec(s) queued for export to Google Docs:`,
    ...lines,
    '',
    `Run the ${skillRef('export', env)} skill now for each: read the spec HTML, create a`,
    'Google Doc via the Google Drive MCP. Start with specforge export-working <id>,',
    'then report the link back with specforge export-done <id> --url <docUrl>',
