...
export function createPublications(deps = {}) {
   ...
   */
  // Counted, not a plain set. A subtree delete holds its whole planned set for
  // the duration and then revokes each spec in turn, so the same id is held
  // twice over; a set would have the inner release drop the outer hold and
  // reopen the window this exists to close.
  const deletingCount = new Map();
  const deleting = {
    has: (id) => deletingCount.has(id),
    add(id) { deletingCount.set(id, (deletingCount.get(id) || 0) + 1); },
    delete(id) {
      const n = deletingCount.get(id);
      if (n === undefined) return;
      if (n <= 1) deletingCount.delete(id);
      else deletingCount.set(id, n - 1);
    },
  };
  ...
  async function bindGateway(want) {
    ...
    const srv = serverImpl(resolve, resolveProject);
   ...
   *
   * Existence is re-checked per request too: a rename in the UI moves every
   * spec to the new name with the daemon still running, and the old token must
   * die then, not at the next restart. The scan is the same one the project
   * page performs to render, so the cost class does not change.
   */
  function resolveProject(token) {
    if (!isToken(token)) return null;
    const name = publishedProjects.get(token) || null;
    if (!name) return null;
    return projectExists(name) ? name : null;
  }

  /**
   * Unshare every published project that no longer exists.
   *
   * The live-session half of the restore() prune: the daemon calls this after
   * an organize or prefs write, which are the only operations that can empty a
   * project, so the tunnel retires without waiting for a restart. Never throws;
   * it runs behind responses nobody is waiting on.
   */
  async function sweepProjects() {
    for (const name of [...projectTokens.keys()]) {
      try {
        if (!projectExists(name)) await unshareProject(name);
      } catch { /* the next sweep or restart gets it */ }
    }
  }
  ...
  /**
   * Whether a delete is currently holding this spec.
   *
   * The DELETE route revokes every share in the subtree and only then moves the
   * directories, and it re-reads membership before the first move so a spec
   * reparented out in between is not deleted. That spec survives with its share
   * already revoked: a public link broken for a document nobody deleted. The
   * reparent is the half to refuse, because the revocation cannot be undone —
   * the token is gone — and the delete is the operation the reader was told
   * about.
   */
  function isDeleting(id) { return deleting.has(id); }

  /**
   * Hold every spec in a planned deletion for as long as `fn` takes.
   *
   * `unshareThen` holds one spec, and the DELETE route nests them, so a spec
   * deep in the subtree is unheld until the revokes reach it — which is exactly
   * the window a reparent needs to take it out of the deletion. The whole set is
   * claimed before the first revoke instead.
   *
   * @param {string[]} ids
   * @param {() => Promise<any>} fn
   */
  async function holdSubtree(ids, fn) {
  ...
  return {
    share, unshare, unshareThen, isDeleting, holdSubtree,
    list, isLive, resolve, restore, stopAll,
