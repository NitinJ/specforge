...
export function createPublications(deps = {}) {
  ...
  function share(id) {
    ...
    // Idempotent only while the publication still works. An entry whose tunnel
    // has died would otherwise hand back a URL that answers 502, which makes
    // "share again" the one action that cannot fix a broken link.
    const existing = live.get(id);
    if (existing && isLive(id)) return Promise.resolve(existing.record);
    const inflight = starting.get(id);
    if (inflight) return inflight;
    const p = (existing ? replaceShare(id) : startShare(id)).finally(() => { starting.delete(id); });
    starting.set(id, p);
    return p;
  }

  /**
   * Tear down a publication whose tunnel is gone, then publish a fresh one.
   *
   * The teardown is inline rather than a call to unshare(), because unshare
   * waits on any in-flight share for this spec and this runs as one.
   */
  async function replaceShare(id) {
    const entry = live.get(id);
    live.delete(id);
    if (entry) {
      try { await entry.stop(); } catch { /* the child is what died */ }
      await new Promise((r) => entry.server.close(r));
    }
    clearShare(id);
    return startShare(id);
  }

  async function startShare(id) {
    if (!readMeta(id)) throw new Error(`unknown spec ${id}`);

    // A record with no live entry is from a daemon that was killed outright:
    // its listener is gone but its tunnel may not be, so the URL answers 502.
    // Publishing over it without reaping that process would leak one tunnel per
    // attempt, and regenerating a dead link is exactly when this happens.
    const orphan = readShare(id);
    if (orphan && orphan.pid) {
      try { killImpl(orphan.pid); } catch { /* already gone */ }
    }
    ...
    live.set(id, { record, server, stop: tunnel.stop, alive: tunnel.alive });
   ...
   *
   * Registry membership is not enough: cloudflared can die on its own, leaving
   * an entry that looks published while the URL answers nothing. Anything that
   * tells a human "this link works" has to ask the tunnel, not the bookkeeping.
   */
  function isLive(id) {
    const entry = live.get(id);
    if (!entry) return false;
    return entry.alive ? !!entry.alive() : true;
  }
  ...
  return { share, unshare, unshareThen, list, isLive, clearStale, stopAll };
}
