 ...
 */
function pidIsCloudflared(pid) {
  try {
    // procfs is authoritative and free. NUL-separated argv, so a substring test
    // is enough to tell cloudflared from whatever else took the number.
...
export function createPublications(deps = {}) {
   ...
   */
  async function stopRecorded(pid) {
  ...
  async function retireIfIdle() {
    ...
    // any stop that could not confirm the process is gone. A stale record costs
    // one reap attempt on the next start; a deleted one costs an untrackable
    // public endpoint.
  ...
  async function adoptRecordedTunnel() {
    ...
    if (!aliveImpl(rec.pid) || !ownsImpl(rec.pid)) {
      await reapRecordedTunnel();
      return false;
    }
    try {
  ...
  async function reapRecordedTunnel() {
    const rec = readTunnel();
    if (!rec) return;
    if (await stopRecorded(rec.pid)) clearTunnel();
  }
