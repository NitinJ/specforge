...
import { storeRoot, isReservedId } from './store-paths.mjs';
...
function styleRanges(html) {
  ...
  while ((open = re.exec(html)) !== null) {
    ...
    out.push({ from, to, css: html.slice(from, to) });
    re.lastIndex = to;
  }
  return out;
}
...
/**
 * The style element the block lives in, or the first one when there is no block
 * yet.
 *
 * A spec can carry more than one stylesheet, and looking only at the first would
 * read an existing block in a later one as absent and insert a second copy.
 */
function styleRange(html) {
  const ranges = styleRanges(html);
  if (!ranges.length) return null;
  return ranges.find((r) => BLOCK_RE.test(r.css)) || ranges[0];
}
...
export function syncAll(opts = {}) {
  ...
  for (const id of readdirSync(root).sort()) {
    // The library document sits under specs/ so the review APIs reach it, and it
    // is opted in, so without this it would be reported as a synced spec. It is
    // regenerated by `components build`, never stamped.
