...
export function parseMarkdown(src) {
  ...
  while (i < lines.length) {
    ...
    if (html && !/^\s*<(?:https?:|[\w.]+@)/.test(line)) {
      ...
      const raw = [line];
      const selfContained = close.test(line) || /\/>\s*$/.test(line);
      i++;
      if (!selfContained) {
        while (i < lines.length && !close.test(lines[i])) { raw.push(lines[i]); i++; }
        if (i < lines.length) { raw.push(lines[i]); i++; }
      }
...
export function inlineToHtml(md) {
  ...
  s = s.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, text, href) => `<a href="${href}">${text}</a>`);
