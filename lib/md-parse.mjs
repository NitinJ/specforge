...
/**
 * True when a URL is safe for an attribute the browser FETCHES on its own
 * (src, poster, an <image> xlink:href), as opposed to one the reader clicks.
 *
 * Stricter than isSafeUrl: no remote host. A spec is self-contained and can be
 * published, so a remote src is a beacon that fires for every reviewer who opens
 * the page and hands the author their IP address.
 */
export function isSafeLoadUrl(value) {
  const decoded = decodeUrl(value);
  if (/^(?:\/\/|[a-z][a-z0-9+.-]*:)/i.test(decoded)) return SAFE_DATA.test(decoded);
  return true; // relative or a fragment
}

/**
 * Escape a value for interpolation into a double-quoted attribute.
 *
 * Quotes only. By the time this runs the string has already been through esc(),
 * so `&`, `<` and `>` are escaped; doing it again turns `&amp;` into `&amp;amp;`
 * and a query string like `?b=1&c=2` renders with the ampersand spelled out.
 */
function escAttrValue(s) {
  return String(s).replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}

/** A link URL safe to put in an href, escaped, or '#'. */
function safeHref(value) {
  return escAttrValue(isSafeUrl(value) ? value : '#');
}
...
export function inlineToHtml(md) {
  ...
  // A hard line break: two or more spaces before a newline. Lifted out with the
  // same sentinel as the code spans, so the trailing spaces survive the
  // whitespace handling between here and the end.
  s = s.replace(/ {2,}\n/g, `${HOLE}br${HOLE}`);
  ...
  // An inline image is a LOAD, not a link: the browser fetches it as soon as the
  // page opens. A remote one in an imported spec is a beacon, so rather than
  // emitting an img nobody wants, the alt text is kept as the text it describes.
  // (A block-level image line goes through the asset resolver instead, which
  // reports what it refused; there is nowhere to report from here.)
  //
  // alt is escaped rather than passed through: esc() above handles &<> but not
  // quotes, so `![" onerror="alert(1)](x)` would close the attribute and add a
  // handler of its own.
  s = s.replace(RE.image, (_m, alt, src) => (
    isSafeLoadUrl(src)
      ? `<img src="${escAttrValue(src)}" alt="${escAttrValue(alt)}">`
      : escAttrValue(alt)
  ));
  s = s.replace(RE.link, (_m, text, href) => `<a href="${safeHref(href)}">${text}</a>`);
  s = s.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
  s = s.replace(/(^|[\s(])\*([^*\s][^*]*?)\*(?=[\s).,;:!?]|$)/g, '$1<em>$2</em>');
  // Autolinks go through safeHref like every other URL. The pattern pins the
  // scheme, but not the quoting: `<https://a"onmouseover="alert(1)>` would
  // otherwise close the href and add a handler after it.
  s = s.replace(/&lt;(https?:\/\/[^\s&]+)&gt;/g, (_m, url) => `<a href="${safeHref(url)}">${url}</a>`);
  // Undo the backslash escapes the exporter writes.
  s = s.replace(/\\([\\`*_[\]#>+.-])/g, '$1');

  return s
    .replace(new RegExp(`${HOLE}br${HOLE}`, 'g'), '<br>\n')
    .replace(new RegExp(`${HOLE}(\\d+)${HOLE}`, 'g'), (_m, n) => spans[Number(n)]);
}
