...
/** A URL safe to put in an attribute, escaped, or '#'. */
...
export function inlineToHtml(md) {
  ...
  // alt is escaped, not passed through: esc() above handles &<> but not quotes,
  // so `![" onerror="alert(1)](x)` would otherwise close the attribute and add a
  // handler of its own.
  s = s.replace(RE.image, (_m, alt, src) => `<img src="${safeHref(src)}" alt="${escAttrValue(alt)}">`);
