...
import { parseMarkdown, inlineToHtml, parseMarker } from './md-parse.mjs';
...
/**
 * Remove what must never reach the review page. Imported markdown is not
 * trusted: the daemon serves the result in a browser, so a <script> that rode in
 * on a raw HTML block would execute there.
 */
export function sanitizeHtml(raw) {
  return String(raw)
    ...
    .replace(/(<[^>]+\bhref\s*=\s*")javascript:[^"]*(")/gi, '$1#$2');
}
...
function renderList(list, ctx) {
  ...
  const items = list.items.map((item) => {
    const inner = item.blocks
      .filter((b) => b.type !== 'marker')
      .map((b) => (b.type === 'list' ? renderList(b, ctx) : renderInlineText(b.text || '')))
      .join(' ');
    ...
    const text = renderInlineText(item.text);
    const continuation = item.blocks
      .filter((b) => b.type === 'paragraph')
      .map((b) => ` ${renderInlineText(b.text)}`)
      .join('');
    return `<li${attrs}>${text}${continuation}${nested}</li>`;
  });
...
function renderBlocks(blocks, ctx) {
  ...
  for (const block of blocks) {
    ...
    switch (block.type) {
      case 'heading':
        html = `<h${Math.min(block.level, 6)}>${renderInlineText(block.text)}</h${Math.min(block.level, 6)}>`;
        break;
      case 'paragraph':
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'list' && block.items.some((i) => i.checked !== null)) {
      for (const item of block.items) {
        ...
        current.tasks.push({ id, status, text, verify: verifyBlock ? verifyBlock.text.replace(/^verify:\s*/i, '') : '' });
      }
      continue;
    }
...
export function toSections(blocks, ctx) {
  ...
  for (const block of blocks) {
    ...
    if (!current) {
      if (!sections.length) open('TL;DR', 'tldr');
    }
...
export function composeShell(shell, { title, status, date, owner, sections }) {
  ...
  const body = sections
    .map((s) => {
      const heading = s.id === 'tldr' && /^TL;DR$/i.test(s.heading)
        ? '<h2>TL;DR</h2>'
        : `<h2>${renderInlineText(s.heading)}</h2>`;
...
export function markdownToSpecHtml(md, opts = {}) {
  ...
  const title = opts.title || parsed.frontmatter.title || (h1 ? stripTags(inlineToHtml(h1.text)) : '') || 'Untitled';
