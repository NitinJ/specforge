...
// Attributes the reader navigates by choice: an ordinary link.
const NAVIGATE_ATTRS = new Set(['href', 'action', 'formaction']);
...
function keepAttrs(attrText) {
  ...
  while ((m = ATTR.exec(attrText))) {
    ...
    if (LOAD_ATTRS.has(lower) && !isSafeLoadUrl(value)) continue;
    const safe = NAVIGATE_ATTRS.has(lower) && !isSafeUrl(value) ? '#' : value;
...
export function sanitizeHtml(raw) {
  ...
  return out.replace(TAG, (_match, closing, tag, attrText) => {
    ...
    return `<${name}${keepAttrs(attrText)}${selfClosing}>`;
  });
}
