...
import { parseMarkdown, inlineToHtml, isSafeUrl } from './md-parse.mjs';
...
//
// The separator is [\s/] and not \s: HTML accepts a solidus where whitespace
// would go, so `<img/src=x/onerror=alert(1)>` is a perfectly ordinary img with
// an event handler, and a pattern anchored on whitespace walks straight past it.
const SEP = '[\\s/]';
const URL_ATTR = new RegExp(
  `(${SEP}(?:href|src|action|formaction|poster|xlink:href)\\s*=\\s*)(?:"([^"]*)"|'([^']*)'|([^\\s>]+))`,
  'gi'
);
const EVENT_ATTR = [
  new RegExp(`${SEP}on[a-z]+\\s*=\\s*"[^"]*"`, 'gi'),
  new RegExp(`${SEP}on[a-z]+\\s*=\\s*'[^']*'`, 'gi'),
  new RegExp(`${SEP}on[a-z]+\\s*=\\s*[^\\s>]+`, 'gi'),
];
const SRCDOC_ATTR = new RegExp(`${SEP}srcdoc\\s*=\\s*(?:"[^"]*"|'[^']*'|[^\\s>]+)`, 'gi');
...
export function sanitizeHtml(raw) {
  ...
  // Inline event handlers, in all three quoting forms. Repeated to a fixed point
  // because removing one handler can bring the next one's separator into reach
  // (`<img/onerror=x/onload=y>`), and one pass would leave the second behind.
  for (let i = 0; i < 5; i++) {
    const before = out;
    for (const re of EVENT_ATTR) out = out.replace(re, '');
    out = out.replace(SRCDOC_ATTR, '');
    if (out === before) break;
  }

  // Navigable URLs, whatever quoting and escaping they arrived in.
  return out.replace(URL_ATTR, (match, prefix, dq, sq, uq) => {
