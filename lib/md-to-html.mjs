...
import { computeTracker, renderTrackerTable, renderStageTable } from './tracker.mjs';
...
function renderPlan(blocks, ctx) {
  ...
  const html = stages
    .map((s) => {
      const tasks = s.tasks
        .map((t) => {
          // The note is a nested list item, not a trailing span: the impl
          // template writes it that way and the exporter reads both.
          const verify = t.verify
            ? `<ul><li class="verify">verify: ${renderInlineText(t.verify)}</li></ul>`
            : '';
      ...
      return [
        ...
        tasks ? `<ol class="sf-tasks">${tasks}</ol>` : '',
        notes,
        '</li>',
      ].join('');
    })
...
/**
 * Add or refresh the two tracker sections from the plan that is already in
 * `html`.
 *
 * Both are written here rather than only refreshed, because the live renderer
 * fills a section it finds and never creates one: markdown carries no tracker,
 * so an imported spec that did not get the sections here would never show them.
 */
function withTracker(html, sections) {
  const tracker = computeTracker(html);
  const stage = [
    '  <section id="stage-tracker" data-sf-section>',
    '    <h2>Stage tracker</h2>',
    '    <p class="sub">One row per stage, projected from the plan. Rendered live when served.</p>',
    `    ${renderStageTable(tracker)}`,
    '  </section>',
  ].join('\n');
  const task = [
  ...
  let out = html;
  for (const [id, section] of [['stage-tracker', stage], ['task-tracker', task]]) {
    const present = new RegExp(`<section\\b[^>]*id="${id}"[\\s\\S]*?</section>`);
    if (present.test(out)) {
      out = out.replace(present, section.trim());
      continue;
    }
    if (!sections.some((s) => s.id === 'impl-plan')) continue;
    // After the plan, and after any tracker already placed there, so the two
    // land in the order they are listed above.
    out = out.replace(/(<section\b[^>]*id="impl-plan"[\s\S]*?<\/section>(\s*<section\b[^>]*id="stage-tracker"[\s\S]*?<\/section>)?)/,
      `$1\n\n${section}`);
  }
  return out;
}
