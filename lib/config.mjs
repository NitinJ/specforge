...
// The canonical design-palette tokens every spec defines (dark `:root` values plus a
// `[data-theme="light"]` override). Enforced by the lint so the review-layer theme
// variants can override ONE known token set instead of guessing per-spec dialects.
// Names are bare (no leading `--`). Overridable via config.json `paletteTokens`.
export const PALETTE_TOKENS = [
  'bg', 'panel', 'panel2', 'ink', 'muted', 'line',
  'accent', 'green', 'amber', 'red', 'code', 'shadow', 'mono',
];
...
export const DEFAULTS = {
  ...
  // The canonical color/design tokens the lint requires (see PALETTE_TOKENS).
