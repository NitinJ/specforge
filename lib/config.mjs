...
// `[data-theme="light"]` override). The lint enforces this exact set so the
// review-layer theme variants can override ONE known token set instead of guessing
// per-spec dialects. Fixed house-wide (specs live in the shared global store, so it
// isn't a per-project knob). Names are bare (no leading `--`).
