...
//     store-api records the pre-authors default, OWNER_DEFAULT below. That name
//     is reserved (mentions.mjs) precisely so a reviewer cannot take it and
//     disappear into the owner's comments — raised in review of PR #219.
//
// Two blind spots, neither fixable from here:
//
//   - An owner who opens their own PUBLISHED link is asked for a name like
//     anyone else, and nothing in the store can tell the name they type from a
//     reviewer's. Telling them apart needs an owner identity the store does not
//     keep.
//   - A store written before `human` was reserved may hold a reviewer with that
//     name, whose comments still read as the owner's. Nothing on disk is ever
//     rewritten, so that stays true for those comments.
 ...
 * `authorFor` default, which in practice means the owner's own browser. It is
 * in RESERVED_NAMES so no reviewer can be recorded under it.
 */
 ...
 * @param {Map<string, object[]>} [threads] already-read threads per spec id, so
 *   a render that has walked the comment stores reads them once (the project
 *   page shares one pass with its collection rank). An id it does not hold is
 *   read here.
 * @returns {{name:string, comments:number, specs:number}[]}
 */
export function projectCollaborators(specIds, threads = null) {
  ...
  for (const id of specIds || []) {
    ...
    for (const thread of (threads && threads.has(id) ? threads.get(id) : loadComments(id).threads) || []) {
