...
//     collection, project, parent, author, reviewers, created, updated }
//
// `author` and `reviewers` are agents, {harness, model}, written only through
// lib/credits.mjs and absent on specs that predate them.
//
// `project` and `collection` together are the spec's address on the home page,
// and `parent` is the spec it belongs to, which decides where its row is drawn
// under that address: a child is drawn indented beneath its parent rather than
// at the top level. All three default to null and all three are read as
// `m.field || null`, so a meta.json written before any of them existed needs no
// migration and no repair pass.
//
// `parent` is the store's only spec-to-spec reference. It is stored on the child
// and nowhere else: there is no `children` array, because a second copy of the
// same fact can disagree with the first after an interrupted write. The reverse
// direction is a scan, in lib/spec-tree.mjs, which is the only module that walks
// this field.

import { mkdirSync, readFileSync, writeFileSync, readdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { specDir, metaPath, specsDir, isReservedId } from './store-paths.mjs';
