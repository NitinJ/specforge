...
function entry(c) {
  ...
  // Most notices have no example of their own, so the specimen renders the rule
  // itself. Printing it above as prose as well says the same sentence twice and
  // doubles the length of a page whose job is to be scanned.
