...
/** Reserved store id. Not a spec id: nothing can create or delete a spec here. */
export const DOC_ID = 'specforge-components';

export function docPath() {
  return join(storeRoot(), 'components', 'components.html');
}
...
${buildCss().trim()}
  ...
  *{box-sizing:border-box}
  ...
  main{max-width:1100px;margin:0 auto;padding:40px 32px 120px}
  h1{font-size:30px;margin:0 0 6px}
  h2{font-size:19px;margin:44px 0 6px;padding-top:14px;border-top:1px solid var(--line)}
  .sub{color:var(--muted);font-size:14px;margin:0 0 14px}
  .cmp{display:grid;grid-template-columns:280px minmax(0,1fr);gap:22px;align-items:start;
    padding:16px 0;border-top:1px solid var(--line)}
  .cmp-name{font-size:13px}
  .cmp-rule{margin:8px 0 0;font-size:13.5px}
...
export function writeDoc() {
  const path = docPath();
  const html = buildDoc();
  mkdirSync(join(storeRoot(), 'components'), { recursive: true });
