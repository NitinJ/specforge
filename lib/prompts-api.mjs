...
export function handlePromptsGet() {
  ...
  return {
    language: {
      ...
      // The shipped contract travels so the page can show what the user's
      // direction is being added to. It is not the box's value: a direction
      // extends the contract rather than replacing it (axis A).
