...
import { shippedLanguageContract } from './language-contract.mjs';
...
export function handlePromptsGet() {
  ...
  return {
    language: {
      // Empty means the shipped contract is in force unedited, so the page
      // opens the box on the contract itself. Editing rules you can see beats
      // restating them in a second document that silently outranks the first,
      // which is what the box was before.
      value: stored.language || shippedLanguageContract(),
...
export function handlePromptsPut(patch) {
  ...
  if (typeof p.language === 'string' && p.language.trim() === shippedLanguageContract().trim()) {
