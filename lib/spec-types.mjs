...
export const BUILTIN = {
  general: {
    shell: 'doc',
    whenToUse: 'A general document none of the other kinds fits: a proposal, a policy, '
      + 'a postmortem, a runbook, a brief. Scaffolds the chrome and a TL;DR, and you '
  ...
  research: {
    shell: 'doc',
    whenToUse: 'Research answering one specific question that has a right answer: '
      + 'method, findings, evidence, a verdict. The question is known and the answer '
      + 'is not. Pick exploration-spec instead when you cannot yet state the question, '
      + 'because the subject is a space to map rather than a question to settle, and '
      + 'code-exploration-spec when the subject is code we own.',
  },
  deck: {
    shell: 'doc',
    whenToUse: 'A deck: slides, in order, each carrying one point. Pick this when the '
      + 'artifact is presented rather than read, and a document kind for anything that '
      + 'has to survive being read alone.',
  },
  'design-impl': {
    shell: 'impl',
...
// The document kinds join the table from their own definitions, which is where
// their sections and prompts already live. Kept as one table rather than two
// lists so that everything downstream keeps asking one question: nothing else
// needs to know whether a kind was written here or beside its sections.
//
// `promptOnly` marks a kind that is already in the table above and is here for
// its section guidance alone, so it does not overwrite what it already has.
for (const d of DOCUMENT_TYPES) {
  if (d.promptOnly) continue;
  BUILTIN[d.slug] = { shell: d.shell || 'doc', whenToUse: d.whenToUse };
}
...
/**
 * Stored rows that name a kind the plugin now ships, and are therefore ignored.
 *
 * A store written before a kind shipped can hold a custom row for its slug, and
 * `customTypes` drops it: two definitions for one slug would give one template
 * spec two owners (I1), and the built-in has to win or an upgrade could not add
 * a kind at all.
 *
 * Dropping it is right and dropping it quietly is not. The row carried a
 * description its author wrote, and that description is now replaced by the
 * shipped one without anyone saying so. Their template spec survives, because
 * the store copy still wins over the bundled shell, so nothing is lost that
 * cannot be read; what is lost is the knowledge that it happened.
 *
 * Reported rather than logged. This is read where someone is already looking at
 * the type list, and a warning printed from a function called on every read
 * would be noise everywhere else.
 *
 * @returns {{slug:string, whenToUse:string}[]}
 */
export function shadowedTypes() {
  let raw;
  try {
    raw = JSON.parse(readFileSync(typesPath(), 'utf8'));
  } catch {
  ...
  for (const row of raw.custom) {
    ...
    if (!kind || seen.has(kind.slug)) continue;
    seen.add(kind.slug);
    if (Object.hasOwn(BUILTIN, kind.slug)) out.push({ slug: kind.slug, whenToUse: kind.whenToUse });
  }
  return out;
}
