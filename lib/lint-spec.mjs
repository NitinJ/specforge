...
// Derived from the component definitions rather than listed here. This was a
// hand-kept list, and it was already wrong: `.evidence` is a block component and
// tripped this check as trapped content because nobody updated both places. Any
// block-level component the library adds is commentable by construction now.
// The extras are pre-library shapes specs still use and the library does not own.
...
export function lintSpec(html) {
  ...
  // Advisory: the component library (references/spec-components.md). Runs only
  // on specs that opted in, so the 113 pre-library specs report nothing.
  ...
  if (comp.applies) {
    checks.push({
      ...
      advisory: true,
      detail: comp.problems.length === 0
        ? 'every component is in the library'
        : comp.problems.join('; '),
    });
  }
