...
const LANGUAGE_RULES = [
  { name: 'em dash', re: /—/g, fix: 'use a colon, semicolon, parentheses, or split the sentence' },
  {
    ...
    re: /\b(worth noting|it is worth|the finding that matters|note that|importantly|interestingly|crucially)\b/gi,
    fix: 'rank by a severity field, not an adjective',
  },
  {
    name: 'precision theatre',
    re: /\b(typically \d|roughly \d|a handful of|a bounded number of|several days|most (?:of them|cases|users))\b/gi,
