...
import { sessionKey } from './harness-context.mjs';
...
/**
 * Where a deleted spec goes.
 *
 * Deletion used to unlink the directory, which made it unrecoverable. A delete
 * that takes a whole subtree needs an undo, so the directories are moved here
 * instead, grouped by the deletion that moved them.
 */
export function trashDir() {
  return join(storeRoot(), 'trash');
}

/** The directory holding everything one delete action moved. */
export function trashDeletionDir(deletionId) {
  assertSpecId(deletionId);
  return join(trashDir(), deletionId);
}

/** Where one spec's directory sits inside a deletion. */
export function trashSpecDir(deletionId, specId) {
  assertSpecId(specId);
  return join(trashDeletionDir(deletionId), specId);
}

/** The record naming every spec one delete action moved. */
export function trashRecordPath(deletionId) {
  assertSpecId(deletionId);
  return join(trashDir(), `${deletionId}.json`);
}
