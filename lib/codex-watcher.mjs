...
export async function watchCodex(session, {
  ...
  try {
    ...
    while (specsForSession(session).length && workerFor(session)?.leaseId === worker.leaseId) {
      const work = pendingWorkForSession(session, { ...env, SPECFORGE_HARNESS: 'codex' });
      const keys = work ? work.items.map((item) => JSON.stringify([work.kind, item])) : [];
      try {
        if (keys.some((key) => !sent.includes(key))) await deliver(session, work.reason);
        sent = keys;
