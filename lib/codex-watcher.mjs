...
export async function watchCodex(session, {
  ...
  let sent = [];
  try {
    sent = JSON.parse(readFileSync(sentPath, 'utf8')).map((key) => {
      const [kind, item] = JSON.parse(key);
      // Earlier workers persisted [kind, item] rather than the request identity.
      return item && typeof item === 'object' ? deliveryKey(kind, item) : key;
    });
  } catch { /* first start */ }
  try {
