...
This was wrong until it was measured. `.sf-asides` set the review layer's chrome font on the container, so spec prose in the panel rendered at 14px/1.5 where the document said 16px/1.7. Colour was never affected, because every `--sf-*` token aliases the spec's own palette, which is also why an aside re-themes with the document. A browser test now compares an aside element by element against identical markup left in the flow.
...
- [x] 11.7 The panel stops imposing its chrome font on the draft. The container declares no font; the chrome inside it declares its own.
      verify: in a real browser, an aside's prose, headings and monospace runs match identical markup left in the flow on family, size, line height, weight and colour; the panel header and the buttons still differ from the document.
...
- **The panel was styling spec content as chrome.** `.sf-asides` carried the review layer's font, and an aside is a section of the spec, so its prose rendered at 14px/1.5 against the document's 16px/1.7. Nobody saw it across two stages of building the panel, because it looks deliberate: a slightly smaller draft beside a document reads as a design choice rather than as a bug. Found by comparing an aside against identical markup left in the flow instead of looking at it.
