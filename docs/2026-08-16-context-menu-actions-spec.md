...
| Actions on it | **&#8592; Import into spec** and **Delete**, in the header strip. They split on whether answering needs judgement. Import is a comment like every other action ([§9](#how-it-runs)): it sends `@agent @import` on the aside. Delete is not: the browser calls the endpoint and the section goes. |
| Import | Lands the draft in the section named in `data-sf-aside`, around the block named in `data-sf-block`, and deletes the aside. What landing means is the originating action's `importInstruction`: a diagram supersedes the prose it was drawn from, a plain-language rewrite sits beside it, a verification report imports as corrections rather than as prose. How much of the section a draft supersedes is the agent's call, under one rule: cut only what the draft carries forward. Threads on the imported blocks travel with them, because they anchor to blocks rather than to the aside. |
| Delete | `DELETE /api/spec/:id/aside/:asideId` removes the section and its threads, asked for confirmation first. No agent, no wait. The endpoint refuses a section without `data-sf-aside`, so the only thing it can remove is a draft SpecForge wrote. |
...
> **Import goes through the agent; Delete does not.** The line between them is whether answering takes judgement. Importing a draft means deciding what it supersedes and where it belongs, which is the work [the guidance above](#asides) exists to direct, so it costs a round trip and buys placement. Rejecting one takes no decision at all: there is nothing to word and nothing to place, so a comment round trip bought a wait and a token bill for a delete.

#### Delete is the one click that changes the document

Inline section editing was removed from SpecForge in v0.2.47 on the grounds that a browser editing spec source is not what the review layer is for, so a browser-driven delete needs the distinction stated. The removed feature sent **markup** the reader typed and applied it to a section the reader wrote. This sends **an id** and removes a section SpecForge itself wrote. Nothing is read from the client but which aside, and `rename` has been writing the spec's `<h1>` from an endpoint since long before this.

| Guard | What it stops |
| --- | --- |
| The section must carry `data-sf-aside` | A request naming any other section id. This is the whole design: without it the route deletes the reader's own writing on a bad id. It lives in the splicer rather than in the handler, so it protects every caller and not just the route. |
| Threads go before the section | A thread left anchored to an id that is gone, which the reconcile then re-attaches to whatever text is nearest. A comment that comes back attached to something the reader never wrote it about is worse than one deleted with its draft. |
| Nothing is written until the delete is legal | Half of it being done. A refused request leaves `spec.html` and `comments.json` as they were. |
| The reader confirms | A draft going for good on a misclick. Nothing in the store is versioned, so getting it back costs an agent round trip. Same dialog an unshare uses. |
| Templates are refused, as on every write route | An aside on a template spec being removed by a reader looking at the scaffolding source. |

The browser removes no nodes of its own. The write triggers the live reload the spec file already has, and the aside goes because it is no longer in the file, so there is one source of truth rather than a client copy that can disagree with it. The panel closes first, since it is open on a section about to stop existing.

#### Every action says how its own output is imported

An action carries two instructions. `instruction` says what to write, and `importInstruction` says what folding it back in means. They are separate standards because the second does not follow from the first: Visualize and Show an example both produce a figure, and one supersedes the prose it came from while the other adds to it.

| Action | What importing it does |
| --- | --- |
| Explain simply | Adds, never replaces. The precise wording of a spec is usually load bearing and a plain-language version is a second way in rather than a better one. |
| Visualize | Supersedes the blocks the diagram carries forward, and only those. Three paragraphs of twelve become the figure; the other nine stay. A diagram covering the section replaces the section, id and heading kept. |
| Go deeper | Adds detail that was not there, after the block it expands. Folded into the section's register rather than pasted underneath it as a second document. |
| Verify against code | Not prose to paste. Each claim the report found wrong is corrected in place, in the section's own words, and the file and line references stay behind. A report that found nothing imports nothing. |
| Help me decide | The decision, not the options. It replaces whatever the section said while the question was open. With no answer given yet there is nothing to import, so the agent asks. |
| Show an example | Adds after the concept it illustrates, marked illustrative, cutting nothing. An example is not a replacement for stating the rule. |

The agent decides how much of a section a draft supersedes, because that is a judgement about the draft rather than a property of the action that wrote it. One rule bounds it: **cut only what the draft carries forward**. Replacing three paragraphs with a diagram covering those three is right; replacing twelve with it loses nine.

#### What an import resolves against

Import is the one action whose instruction cannot be written in advance. Every other entry in the registry carries a fixed standard; this one acts on a draft that did not exist when the registry was written, so what it is and where it lands are properties of the aside rather than of the action. The aside records all three: `data-sf-aside` names the section, `data-sf-block` names the block, `data-sf-action` names the action whose `importInstruction` says what to do. `importTarget(html, asideId, {bids})` reads them and returns the concrete sentence with that guidance already in it, delivered as `target` on the resolved thread ([§9](#how-it-runs)).
...
| --- | --- |
| Block known | Import into the named section, around the named block it was asked about. |
| No block | Import into the named section, where it belongs rather than appended. |
| `data-sf-action` the registry no longer knows | No guidance to give: fold it in cutting nothing, and say so in the reply. Guessing at a verb would cut prose on the strength of a string nobody recognises. |
| The named section is no longer in the spec | `resolved: false`. No import at all: say the source is gone and ask whether the draft belongs elsewhere or should be deleted. |
| The named block is no longer in the registry | Dropped from the sentence, and the placement falls back to "where it belongs". Same fallback the aside's marker already takes. |
...
> **What replaces what is a judgement, not a setting.** The first version of this shipped a per-action `importMode` of `merge` or `replace`, with Visualize the one `replace`. Two words could not carry six behaviours, and a section-scoped replace threw away prose the diagram never covered. The flag is gone: each action states what importing its own output means, and the agent decides the extent against the draft in front of it.

#### The draft reads as the spec; the panel around it does not

An aside is a section of the spec, so its prose renders exactly as the rest of the spec's prose renders: same family, size, line height, weight and colour, for paragraphs, headings and monospace runs alike. The panel is chrome and reads as chrome. The line is drawn by which element declares a font: the panel container declares none, so content inherits the document, and every piece of chrome inside it (the panel header, the per-aside strip, the buttons, the close control) declares its own.

This was wrong twice, and both were found by measuring rather than looking. `.sf-asides` set the review layer's chrome font on the container, so spec prose in the panel rendered at 14px/1.5 where the document said 16px/1.7. And the reader's chosen reading face never arrived: `applyFont` stamps `data-sf-font` and `--sf-reading-font` on the width container, and `buildAsides` moves the aside out of that container into a fixed panel, so a draft rendered in the default face beside a document rendering in Lora. The same held for the code face and `--mono`.

Both are one defect: the panel is a second place document content lives, and anything applied to the container has to reach it too. `typeRoots()` is now that list, and the panel re-applies the current choice when it is built, since it is built after the font is. Colour was never affected, because every `--sf-*` token aliases the spec's own palette, which is also why an aside re-themed correctly from the start. Browser tests compare an aside element by element against identical markup left in the flow, once at the default face and once with a reading font chosen.
...
- Nothing is open. Q1 to Q8 were settled in review and Q9 during implementation; the answers are the decisions in [§11](#decisions).
...
- [x] **Q9 — resolved** Should a replace ever act on blocks rather than the whole section? Yes, and the agent decides which per import, under one rule: cut only what the draft carries forward. The `merge`/`replace` flag is gone and each action states what importing its own output means. [D8](#decisions).
...
- [x] 10.2 All six aside actions state what importing their own output means, and no two are the same.
      verify: every aside action's guidance is longer than a label and distinct from every other's.
- [x] 10.3 `importTarget` returns `{resolved, guidance}` in place of `mode`, puts the originating action's guidance into the sentence the agent reads, and carries the rule that bounds it.
      verify: the resolved sentence contains the action's own guidance verbatim, names which action it came from, and says to cut only what the draft carries forward.
- [x] 10.4 The skill says an import depends on what kind of draft it is, and restates no guidance of its own.
      verify: a test asserts the skill names `target` and the cut rule, and that no action's import guidance is pasted into it.
...
- [x] 11.2 `deleteAside(html, asideId)` removes an aside and refuses a section that is not one.
      verify: a plain section and an unknown id both throw; an aside holding a nested section is removed whole with no orphaned close tag; an id holding a regex character matches literally.
- [x] 11.3 `removeAside(id, asideId)` deletes the threads before the section, and writes nothing when the delete is refused.
      verify: threads on the aside go and threads on its source stay; a refused delete leaves both files untouched.
- [x] 11.4 `DELETE /api/spec/:id/aside/:asideId`, guarded like every other write route.
      verify: 200 names what went; a non-aside and an unknown section are 400; an unknown spec is 404; a template is 403 and keeps its aside.
- [x] 11.5 The action becomes `delete`, kind `direct`, and the button asks before calling the endpoint. The jsdom harness captures DELETE, which it did not.
      verify: the click opens no composer; nothing is sent until the dialog is answered; confirming calls the endpoint for that aside; a refused delete says so and leaves the panel open. In a real browser, the section leaves the file on disk.
- [x] 11.6 The spec and the skill say Delete never reaches an agent.
      verify: the skill names no action the registry lacks, and the lint passes.
- [x] 11.7 The panel stops imposing its chrome font on the draft. The container declares no font; the chrome inside it declares its own.
      verify: in a real browser, an aside's prose, headings and monospace runs match identical markup left in the flow on family, size, line height, weight and colour; the panel header and the buttons still differ from the document. With a reading font and a code font chosen, the draft follows both.
- [x] 11.8 A malformed aside id is a 400 rather than an `uncaughtException`, and the spec is written before the threads so a failed write cannot lose comments.
      verify: `DELETE …/aside/%zz` answers 400 and the daemon serves the next request; a spec write that fails leaves both the draft and its comments in place.
...
- **A second instruction per action, rather than a mode the resolver interprets.** The alternative was keeping a small vocabulary of verbs and letting the resolver compose the sentence: `replace-blocks`, `append-after`, `correct-in-place`. It stays machine readable and would let the gate check that an import did what it said. Rejected because the vocabulary grows one verb per action until it is prose with a worse syntax, and because the interesting part of Verify against code's import is not "correct in place" but which references to leave behind. Prose that reaches the agent whole beats a token it has to expand.
...
- **"The browser never writes the spec file" was already untrue when this feature cited it.** [§10](#asides) justified Import going through the agent partly on that rule, and `store-api.mjs` said the server never parses a spec. Both were overstated: `POST /api/spec/:id/rename` has been parsing a spec and rewriting its `<h1>` for far longer. The rule that actually held is narrower, and is what the section editor removed in v0.2.47 broke: no spec *source* comes from the client. Delete keeps that rule and the comments were corrected rather than the design bent around a sentence that was never accurate.
- **The panel was styling spec content as chrome, and the reading font never reached it.** `.sf-asides` carried the review layer's font, so an aside's prose rendered at 14px/1.5 against the document's 16px/1.7. Separately, `applyFont` stamps the reader's chosen face on the width container and `buildAsides` moves the aside out of it, so a draft sat in the default face beside a document in Lora. Both survived two stages of building this panel because they look deliberate: a slightly smaller draft in a different face reads as a design choice rather than a bug. Found by comparing an aside element by element against identical markup left in the flow, which is the only way either was ever going to surface.
- **A malformed aside id would have taken the daemon down.** `decodeURIComponent` throws on `%zz`, nothing wraps the router, and an uncaught synchronous throw in a request handler ends the process for every spec open in every tab. Caught in review, not by a test, and now covered by one that asserts the daemon still answers afterwards.
- **The delete wrote comments before the spec, and it should be the other way round.** The first version deleted threads first, reasoning that an orphaned thread is worse than a deleted one. That is true of the steady state and the wrong question for a failure: if the spec write then fails, the draft is still on screen and its comments are gone for good, with nothing in the store versioned to get them back. Spec first leaves orphans, which are visible and fixable. Neither order is atomic and the note in `remove-aside.mjs` says so.
- **The jsdom harness did not capture DELETE.** A client `DELETE` fell through to the read branch, whose stub response carries no `ok`, so code checking `Response.ok` saw `undefined` and reported a failure the server never sent. Found by writing the first test that issues one. The existing unshare path had the same exposure and no test that would have shown it.
- **Stage 9's `importMode` lasted one stage.** It shipped as a per-action `merge`/`replace` flag with Visualize the one `replace`, and the warning recorded against it in [§10](#asides) was the defect: a section-scoped replace discards prose the diagram never covered. Two words could not carry six behaviours either, since Visualize and Show an example both produce a figure and only one supersedes anything. Replaced in stage 10 by a per-action `importInstruction` and a judgement the agent makes per import. A flag and an agent's call cannot both decide the same thing, so the flag went.
