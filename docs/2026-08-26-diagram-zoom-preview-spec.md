...
- **The artwork is promoted to its own compositing layer only while a drag is in flight.** `will-change: transform` was on the holder permanently, as a smoothness hint. A composited layer is rasterized once at the scale it had when it was made, and every later scale stretches that bitmap rather than redrawing the vector, so a diagram at 800% was a blur: the resolution the reader opened the preview to escape. A drag changes the offset and not the scale, which is the one case where translating a raster is exact, so the hint moved onto `.sf-zoom-dragging`. It costs nothing to give up: wheel-zooming to 800% holds a 16.7ms median frame and a 23.1ms worst, measured over 36 frames.
...
| 3, again | pass | Re-checked after Nitin reported the artwork going soft under zoom. At 800% a diagram's glyph edges and 1px strokes are drawn from the vector rather than a stretched raster. A 1024x1536 photograph is exact to 100% and interpolates beyond it, which is the source resolution and not the preview. |
