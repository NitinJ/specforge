...
- **A hover of nothing is held while the pointer is on the trigger.** [§4](#design) has `review.js` report the block under the pointer and `zoom.js` follow it, and the trigger is review chrome: the moment the pointer crosses onto it, `review.js` correctly reports a hover of nothing. Acting on that immediately deleted the trigger the instant the reader reached for it, so the feature could not be used with a real mouse at all. The clear is now held until the pointer leaves the trigger for something that is not its block. The trigger has `mouseenter` and `mouseleave` of its own, which is the second listener [D2](#decisions) was written to avoid, and it is confined to the button rather than the document.
