...
- **Answering [Q1](#open-questions): on a touch device the trigger is always visible, at 60% opacity.** There is no hover to reveal it with. It ships as one `@media (pointer: coarse)` block and no new interaction, which costs less than carrying the question. Agent, 2026-08-26.
...
| --- | --- | --- |
| 1, 2 | pass | The trigger appears over the diagram's top-right corner and is the topmost element at its own centre. Clicking it opens the artwork at its authored 1463x222, centred, scale 1, over a backdrop at 85% black. No composer. |
...
Five defects reached the browser, and none of them was visible to a green suite: jsdom runs no layout and dispatches no real pointer, so a rect is always zero and a wheel never scrolls anything. The trigger did not follow the page when it scrolled; a mermaid diagram opened smaller than it was in the document; the artwork's black edges were invisible over the black backdrop; a wheel over the backdrop scrolled the document behind; and no image in the store could be previewed at all. Each is now covered by a test written from what the browser showed.

One thing the walk could not do by hand: Playwright's own mouse moves off the block on its way to the trigger, the hover clears, and the trigger hides before the click lands. Hit-testing was confirmed separately with `elementFromPoint`, which reports the trigger's own icon as the topmost element at the trigger's centre.
